...
type Uplink struct {
	...
	APIKey map[storx.NodeID]*macaroon.APIKey
	Access map[storx.NodeID]*uplink.Access
	User   map[storx.NodeID]UserLogin
...
func (planet *Planet) newUplink(ctx context.Context, index int, log *zap.Logger, name string) (_ *Uplink, err error) {
	...
	for j, satellite := range planet.Satellites {
		...
		if config.DefaultPathCipher == storx.EncUnspecified {
...
func (client *Uplink) ID() storx.NodeID { return client.Identity.ID }
...
func (client *Uplink) Upload(ctx context.Context, satellite *Satellite, bucket string, path storx.Path, data []byte) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (client *Uplink) UploadWithExpiration(ctx context.Context, satellite *Satellite, bucketName string, path storx.Path, data []byte, expiration time.Time) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (client *Uplink) Download(ctx context.Context, satellite *Satellite, bucketName string, path storx.Path) (_ []byte, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (client *Uplink) DownloadStream(ctx context.Context, satellite *Satellite, bucketName string, path storx.Path) (_ io.ReadCloser, cleanup func() error, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (client *Uplink) DownloadStreamRange(ctx context.Context, satellite *Satellite, bucketName string, path storx.Path, start, limit int64) (_ io.ReadCloser, cleanup func() error, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (client *Uplink) DeleteObject(ctx context.Context, satellite *Satellite, bucketName string, path storx.Path) (err error) {
	defer mon.Task()(&ctx)(&err)
