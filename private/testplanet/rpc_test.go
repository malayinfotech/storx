...
func TestDialNodeURL(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		whitelistPath, err := planet.WriteWhitelist(storx.LatestIDVersion())
		require.NoError(t, err)

		unsignedIdent, err := testidentity.PregeneratedIdentity(0, storx.LatestIDVersion())
		require.NoError(t, err)

		signedIdent, err := testidentity.PregeneratedSignedIdentity(0, storx.LatestIDVersion())
		require.NoError(t, err)
...
func TestDialNode_BadServerCertificate(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		Reconfigure: testplanet.Reconfigure{
			...
			Identities: func(log *zap.Logger, version storx.IDVersion) *testidentity.Identities {
		...
		ident, err := testidentity.PregeneratedSignedIdentity(0, storx.LatestIDVersion())
		require.NoError(t, err)
