  ...
  try {
    ...
    if (env.BRANCH_NAME == "main") {
        stage('Run Versions Test') {
            ...
            try {
              ...
              env.STORX_SIM_POSTGRES = 'postgres://postgres@postgres:5432/teststorx?sslmode=disable'
              env.STORX_SIM_REDIS = 'redis:6379'

              echo "STORX_SIM_POSTGRES: $STORX_SIM_POSTGRES"
              echo "STORX_SIM_REDIS: $STORX_SIM_REDIS"
              ...
              sh '''until $(docker logs postgres-$BUILD_NUMBER | grep "database system is ready to accept connections" > /dev/null)
                ...
                '''
              sh 'docker exec postgres-$BUILD_NUMBER createdb -U postgres teststorx'
              // fetch the remote main branch
              sh 'git fetch --no-tags --progress -- https://github.com/storx/storx.git +refs/heads/main:refs/remotes/origin/main'
              sh 'docker run -u $(id -u):$(id -g) --rm -i -v $PWD:$PWD -w $PWD --entrypoint $PWD/scripts/tests/testversions/test-sim-versions.sh -e STORX_SIM_POSTGRES -e STORX_SIM_REDIS --link redis-$BUILD_NUMBER:redis --link postgres-$BUILD_NUMBER:postgres -e CC=gcc storxlabs/golang:1.19.6'
            }
            catch(err){
                throw err
            }
    ...
    stage('Run Rolling Upgrade Test') {
        ...
        try {
          ...
          env.STORX_SIM_POSTGRES = 'postgres://postgres@postgres:5432/teststorx?sslmode=disable'
          env.STORX_SIM_REDIS = 'redis:6379'
          env.STORX_MIGRATION_DB = 'postgres://postgres@postgres:5432/teststorx?sslmode=disable&options=--search_path=satellite/0/meta'

          echo "STORX_SIM_POSTGRES: $STORX_SIM_POSTGRES"
          echo "STORX_SIM_REDIS: $STORX_SIM_REDIS"
          echo "STORX_MIGRATION_DB: $STORX_MIGRATION_DB"
          sh 'docker run --rm -d -e POSTGRES_HOST_AUTH_METHOD=trust --name postgres-$BUILD_NUMBER postgres:12.3'
          sh 'docker run --rm -d --name redis-$BUILD_NUMBER redis:latest'
          ...
          // fetch the remote main branch
          sh 'git fetch --no-tags --progress -- https://github.com/storx/storx.git +refs/heads/main:refs/remotes/origin/main'
          sh 'docker run -u $(id -u):$(id -g) --rm -i -v $PWD:$PWD -w $PWD --entrypoint $PWD/scripts/tests/rollingupgrade/test-sim-rolling-upgrade.sh -e BRANCH_NAME -e STORX_SIM_POSTGRES -e STORX_SIM_REDIS -e STORX_MIGRATION_DB --link redis-$BUILD_NUMBER:redis --link postgres-$BUILD_NUMBER:postgres -e CC=gcc storxlabs/golang:1.19.6'
        }
        catch(err){
            throw err
        }
