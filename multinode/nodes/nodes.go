...
type DB interface {
	...
	Get(ctx context.Context, id storx.NodeID) (Node, error)
	...
	Remove(ctx context.Context, id storx.NodeID) error
	// UpdateName will update name of the specified node in database.
	UpdateName(ctx context.Context, id storx.NodeID, name string) error
}
