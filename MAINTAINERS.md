...
Example: [Release v1.31](https://storxlabs.atlassian.net/wiki/spaces/ENG/pages/1812791357/Release%2Bv1.31)
Here we need to post changes for each topic(storx-sim, Uplink, Sattelite, Storage Node, General etc.)
...
The next step is to create tag for `storx` repo using `tag-release.sh` which is in storx/scripts folder and push it.
Example:
`./scripts/tag-release.sh v1.3.0-rc`
`git push origin v1.3.0-rc`
Then verify that the Jenkins job of the build Storx V3 for such tag and branch has finished successfully.
...
Verify that the Jenkins job of the build Storx V3 for such tag has finished successfully.
...
```
git clone git@github.com:storx/tardigrade-satellite-theme
...
After Jenkins job for this release finished it will automaticaly post this tag on [GitHub release page](https://github.com/storx/storx/releases). The status will be `Draft`.
...
## Useful links
[Deployment documentation](https://storxlabs.atlassian.net/wiki/spaces/OPS/pages/153190401/Satellite+-+post+phoenix#satellite.qa.storx)
[Storage node rollout process](https://storxlabs.atlassian.net/wiki/spaces/OPS/pages/138084357/Storagenode)
