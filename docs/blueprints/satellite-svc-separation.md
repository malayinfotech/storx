...
GC iterates over the metainfo and creates a list for each storage node. It sends these list to storage nodes, who can delete all pieces missing from the list. See [GC design doc](https://github.com/storx/storx/blob/main/docs/design/garbage-collection.md) for more details. 

#### accounting
Accounting calculates how much uplinks use storage and how much storage nodes store data. See [docs on accounting](https://github.com/storx/docs/blob/main/code/payments/Accounting.md) for more details.
...
#### kademlia
Kademlia, discovery, bootstrap, and vouchers are being removed and not included in this doc. See [kademlia removal design doc](https://github.com/storx/storx/blob/main/docs/design/kademlia-removal.md) for more details.
...
- https://github.com/storx/docs/blob/main/code/Database.md#database
- https://github.com/storx/docs/blob/main/code/persistentstorage/Databases.md
...
The audit observer uses the segments from the metainfo loop to create segment reservoir samples for each storage node and saves those samples to a reservoir cache. Audit observer currently runs on a 30s interval for the release default setting. See [audit-v2 design](https://github.com/storx/storx/blob/main/docs/design/audit-v2.md) for more details.
...
#### audit workers
The audit process should be able to run many audits in parallel. See the [audit-v2 design doc](https://github.com/storx/storx/blob/main/docs/design/audit-v2.md) for updates to the audit design.
...
We will need to add a `Satellite` in `testplanet` so that we can test continue using the same unit tests for the Satellite as we break it apart. See an example of that [here](https://github.com/storx/storx/pull/2836/files#diff-c2ce7a2c9b2f4920e2de524c0fffd2f1R70).
...
- update storx-sim so that integration tests still pass
...
- Update storx-sim to run with all the new services.
