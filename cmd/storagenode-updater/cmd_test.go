...
func TestAutoUpdater(t *testing.T) {
	...
	oldBin := CompileWithVersion(ctx, "storx/cmd/storagenode-updater", oldInfo)
	...
	newBin := CompileWithVersion(ctx, "storx/cmd/storagenode-updater", newInfo)
...
func CompileWithVersion(ctx *testcontext.Context, pkg string, info version.Info) string {
	ldFlagsX := map[string]string{
		"private/version.buildTimestamp":  strconv.Itoa(int(info.Timestamp.Unix())),
		"private/version.buildCommitHash": info.CommitHash,
		"private/version.buildVersion":    info.Version.String(),
		"private/version.buildRelease":    strconv.FormatBool(info.Release),
	}
