...
func (service *Service) convertAliasToNodeURL(ctx context.Context, alias metabase.NodeAlias) (_ storx.NodeURL, err error) {
	...
	if !ok {
		...
		if !ok {
			...
			if !ok {
				return storx.NodeURL{}, Error.Wrap(err)
			}
			service.aliasMap = latest

			nodeID, ok = service.aliasMap.Node(alias)
			if !ok {
				return storx.NodeURL{}, ErrNoSuchNode.New("no node has alias %d", alias)
			}
		}
		...
		if info.Disqualified != nil || info.ExitStatus.ExitFinishedAt != nil {
			return storx.NodeURL{}, ErrNoSuchNode.New("node %s is no longer on the network", nodeID.String())
		}
