...
type Verifier interface {
	Verify(ctx context.Context, nodeAlias metabase.NodeAlias, target storx.NodeURL, targetVersion string, segments []*Segment, ignoreThrottle bool) (verifiedCount int, err error)
}
...
type Service struct {
	...
	aliasToNodeURL  map[metabase.NodeAlias]storx.NodeURL
...
func (service *Service) loadOnlineNodes(ctx context.Context) (err error) {
	...
	for _, node := range nodes {
		...
		service.aliasToNodeURL[alias] = storx.NodeURL{
			ID:      node.ID,
			Address: addr,
		}
	}
...
func (service *Service) parseNodeFile(path string) (NodeAliasSet, error) {
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		nodeID, err := storx.NodeIDFromString(line)
		if err != nil {
