...
func (service *NodeVerifier) Verify(ctx context.Context, alias metabase.NodeAlias, target storx.NodeURL, targetVersion string, segments []*Segment, ignoreThrottle bool) (verifiedCount int, err error) {
...
func (service *NodeVerifier) verifySegment(ctx context.Context, client *piecestore.Client, alias metabase.NodeAlias, target storx.NodeURL, segment *Segment) (outcome audit.Outcome, err error) {
...
func (service *NodeVerifier) VerifyWithExists(ctx context.Context, alias metabase.NodeAlias, target storx.NodeURL, targetVersion string, segments []*Segment) (verifiedCount int, err error) {
...
func (service *NodeVerifier) verifySegmentsWithExists(ctx context.Context, client pb.DRPCPiecestoreClient, alias metabase.NodeAlias, target storx.NodeURL, segments []*Segment) (err error) {
	defer mon.Task()(&ctx)(&err)

	pieceIds := make([]storx.PieceID, 0, len(segments))
