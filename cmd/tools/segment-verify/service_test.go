...
import (
	...
	"common/testcontext"
	"common/uuid"
	segmentverify "storx/cmd/tools/segment-verify"
...
func TestService_EmptyRange(t *testing.T) {
	...
	metabase := newMetabaseMock(map[metabase.NodeAlias]storx.NodeID{})
...
func TestService_Success(t *testing.T) {
	...
	err := os.WriteFile(config.PriorityNodesPath, []byte((storx.NodeID{1}).String()+"\n"), 0755)
	require.NoError(t, err)

	func() {
		nodes := map[metabase.NodeAlias]storx.NodeID{}
		for i := 1; i <= 0xFF; i++ {
			nodes[metabase.NodeAlias(i)] = storx.NodeID{byte(i)}
		}
...
func TestService_Failures(t *testing.T) {
	...
	func() {
		...
		verifier := &verifierMock{
			offline:  []storx.NodeID{{0x02}, {0x08}, {0x09}, {0x0A}},
...
type metabaseMock struct {
	nodeIDToAlias      map[storx.NodeID]metabase.NodeAlias
	aliasToNodeID      map[metabase.NodeAlias]storx.NodeID
	streamIDsPerBucket map[metabase.BucketLocation][]uuid.UUID
	segments           []metabase.VerifySegment
}

func newMetabaseMock(nodes map[metabase.NodeAlias]storx.NodeID, segments ...metabase.VerifySegment) *metabaseMock {
	mock := &metabaseMock{
		nodeIDToAlias:      map[storx.NodeID]metabase.NodeAlias{},
		aliasToNodeID:      nodes,
		segments:           segments,
...
func (db *metabaseMock) Get(ctx context.Context, nodeID storx.NodeID) (*overlay.NodeDossier, error) {
...
type verifierMock struct {
	allSuccess bool
	fail       error
	offline    []storx.NodeID
	success    []uuid.UUID
	notFound   []uuid.UUID

	mu        sync.Mutex
	processed map[storx.NodeID][]*segmentverify.Segment
}

func (v *verifierMock) Verify(ctx context.Context, alias metabase.NodeAlias, target storx.NodeURL, targetVersion string, segments []*segmentverify.Segment, _ bool) (int, error) {
	v.mu.Lock()
	if v.processed == nil {
		v.processed = map[storx.NodeID][]*segmentverify.Segment{}
	}
