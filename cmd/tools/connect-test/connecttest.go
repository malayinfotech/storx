...
func main() {
	...
	var (
		...
		quicNodeID storx.NodeID
		quicErr    error
		tcpNodeID  storx.NodeID
		tcpErr     error
	)
	group.Go(func() error {
...
func tryConnect(ctx context.Context, tlsConfig *tls.Config, dialer rpc.Connector, destAddr string) (storx.NodeID, error) {
	...
	if err != nil {
		return storx.NodeID{}, fmt.Errorf("could not get node ID from peer certificates: %w", err)
	}
