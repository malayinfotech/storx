...
func (c *cmdShare) Setup(params clingy.Parameters) {
	...
	c.baseURL = params.Flag("base-url", "The base url for link sharing", "https://link.storxshare.io").(string)
	...
	c.authService = params.Flag("auth-service", "URL for shared auth service", "https://auth.storxshare.io").(string)
...
func createDNS(ctx context.Context, accessKey string, prefixes []uplink.SharePrefix, baseURL, dns string) (err error) {
	...
	var printStorxRoot string
	if key == "" {
		printStorxRoot = fmt.Sprintf("txt-%s\tIN\tTXT  \tstorx-root:%s", dns, bucket)
	} else {
		printStorxRoot = fmt.Sprintf("txt-%s\tIN\tTXT  \tstorx-root:%s/%s", dns, bucket, key)
	}
	...
	fmt.Fprintln(clingy.Stdout(ctx), printStorxRoot)
	fmt.Fprintf(clingy.Stdout(ctx), "txt-%s\tIN\tTXT  \tstorx-access:%s\n", dns, accessKey)
