...
func (ex *external) Setup(f clingy.Flags) {
	...
	ex.tracing.traceAddress = f.Flag(
		"trace-addr", "Specify where to send traces", "agent.tracing.datasci.storx:5775",
		clingy.Advanced,
	).(string)
	...
	ex.events.address = f.Flag(
		"events-addr", "Specify where to send events", "eventkitd.datasci.storx:9002",
		clingy.Advanced,
	).(string)
...
func (ex *external) Wrap(ctx context.Context, cmd clingy.Command) (err error) {
	...
	exp := os.Getenv("STORX_EXPERIMENTAL")
	if exp != "" {
...
// has a legacy bool to have it return the same values that storx/common/fpath.ApplicationDir
// would have returned.
