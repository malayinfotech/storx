...
import (
	...
	"private/cfgstruct"
	"private/process"
	_ "private/process/googleprofiler" // This attaches google cloud profiler.
	"private/version"
...
func init() {
	defaultConfDir := fpath.ApplicationDir("storx", "satellite")
	defaultIdentityDir := fpath.ApplicationDir("storx", "identity", "satellite")
...
func reportsVerifyGEReceipt(cmd *cobra.Command, args []string) (err error) {
	...
	nodeID, err := storx.NodeIDFromString(args[0])
	if err != nil {
...
func cmdRestoreTrash(cmd *cobra.Command, args []string) error {
	...
	undelete := func(node *overlay.SelectedNode) {
		...
		conn, err := dialer.DialNodeURL(ctx, storx.NodeURL{
			ID:      node.ID,
	...
	if len(args) == 0 {
		...
		for _, nodeid := range args {
			parsedNodeID, err := storx.NodeIDFromString(nodeid)
			if err != nil {
				return err
			}
