...
const (
	...
	// to create a port with a consistent format for storx-sim services.
...
// port creates a port with a consistent format for storx-sim services.
...
func networkExec(flags *Flags, args []string, command string) error {
	...
	if command == "setup" {
		if flags.Postgres == "" {
			return errors.New("postgres connection URL is required for running storx-sim. Example: `storx-sim network setup --postgres=<connection URL>`.\nSee docs for more details https://github.com/storx/docs/blob/main/Test-network.md#running-tests-with-postgres")
		}
...
func newNetwork(flags *Flags) (*Processes, error) {
	...
	storxRoot := strings.TrimSuffix(filename, "/cmd/storx-sim/network.go")
	...
	for i := 0; i < flags.SatelliteCount; i++ {
		...
		apiProcess.Arguments = withCommon(apiProcess.Directory, Arguments{
			"setup": {
				...
				"--console.static-dir", filepath.Join(storxRoot, "web/satellite/"),
				...
				"--mail.from", "Storx <yaroslav-satellite-test@storx>",
				"--mail.template-path", filepath.Join(storxRoot, "web/satellite/static/emails"),
				...
				"--admin.static-dir", filepath.Join(storxRoot, "satellite/admin/ui/build"),
			},
			"run": {"api"},
		})
	...
	for i := 0; i < flags.StorageNodeCount; i++ {
		...
		process.Arguments = withCommon(process.Directory, Arguments{
			"setup": {
				...
				"--console.static-dir", filepath.Join(storxRoot, "web/storagenode/"),
				"--server.address", process.Address,
	...
	{ // setup multinode
		...
		process.Arguments = withCommon(process.Directory, Arguments{
			"setup": {
				...
				"--console.static-dir", filepath.Join(storxRoot, "web/multinode/"),
