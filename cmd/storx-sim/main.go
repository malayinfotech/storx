...
type Flags struct {
	...
	// Connection string for the postgres database to use for storx-sim processes
	Postgres string
	Redis    string
...
func main() {
	...
	rootCmd := &cobra.Command{
		Use:   "storx-sim",
		Short: "Storx Network Simulator",
	}

	defaultConfigDir := fpath.ApplicationDir("storx", "local-network")

	configDir := defaultConfigDir
	if os.Getenv("STORX_NETWORK_DIR") != "" {
		configDir = os.Getenv("STORX_NETWORK_DIR")
	}
	...
	rootCmd.PersistentFlags().StringVarP(&flags.Postgres, "postgres", "", os.Getenv("STORX_SIM_POSTGRES"), "connection string for postgres (defaults to STORX_SIM_POSTGRES)")
	rootCmd.PersistentFlags().StringVarP(&flags.Redis, "redis", "", os.Getenv("STORX_SIM_REDIS"), "connection string for redis e.g. 127.0.0.1:6379 (defaults to STORX_SIM_REDIS)")
	...
	toolCmd := &cobra.Command{
		Use:   "tool",
		Short: "tools for working with storx-sim",
	}
