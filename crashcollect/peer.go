...
import (
	...
	"private/debug"
	"storx/crashcollect/crash"
	"storx/private/crashreportpb"
	"storx/private/server"
)

// Config is the global configuration for storx crash collect service.
type Config struct {
	Debug    debug.Config
...
// Peer is the representation of a storx crash collect service.
//
// architecture: Peer
type Peer struct {
	Log      *zap.Logger
...
// New is a constructor for storx crash collect Peer.
...
// Run runs storx crash collect Peer api until it's either closed or it errors.
func (peer *Peer) Run(ctx context.Context) error {
	group, ctx := errgroup.WithContext(ctx)

	// start storx crash collect web api drpc server as a separate goroutine.
	group.Go(func() error {
