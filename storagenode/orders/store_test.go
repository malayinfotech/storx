...
func TestOrdersStore_ListUnsentBySatellite(t *testing.T) {
	...
	expectedArchivedInfos := make(map[storx.SerialNumber]*orders.ArchivedInfo)
...
func storeNewOrders(ordersStore *orders.FileStore, numSatellites, numOrdersPerSatPerTime int, createdAtTimes []time.Time) (map[storx.SerialNumber]*ordersfile.Info, error) {
	actions := []pb.PieceAction{
		pb.PieceAction_GET,
		pb.PieceAction_PUT_REPAIR,
		pb.PieceAction_GET_AUDIT,
	}
	originalInfos := make(map[storx.SerialNumber]*ordersfile.Info)
