...
type DB interface {
	...
	ListUnsentBySatellite(ctx context.Context) (map[storx.NodeID][]*ordersfile.Info, error)
...
func (service *Service) SendOrders(ctx context.Context, now time.Time) {
	...
	errorSatellites := make(map[storx.NodeID]struct{})
...
func (service *Service) settleWindow(ctx context.Context, log *zap.Logger, satelliteID storx.NodeID, orders []*ordersfile.Info) (status pb.SettlementWithWindowResponse_Status, err error) {
	defer mon.Task()(&ctx)(&err)
