...
func OpenWritableUnsent(unsentDir string, satelliteID storx.NodeID, creationTime time.Time) (Writable, error) {
...
func MoveUnsent(unsentDir, archiveDir string, satelliteID storx.NodeID, createdAtHour, archivedAt time.Time, status pb.SettlementWithWindowResponse_Status, version Version) error {
...
func getUnsentFileInfo(filename string) (satellite storx.NodeID, createdHour time.Time, version Version, err error) {
	filename, version = getVersion(filename)

	if !strings.HasPrefix(filename, unsentFilePrefix) {
		return storx.NodeID{}, time.Time{}, version, Error.New("invalid path: %q", filename)
	}
	...
	satelliteID, err := storx.NodeIDFromString(satelliteIDStr)
	if err != nil {
		return storx.NodeID{}, time.Time{}, version, Error.New("invalid path: %q", filename)
	}
...
// V0 will not have ".<version>" at the end of the filename.
func getArchivedFileInfo(name string) (satelliteID storx.NodeID, createdAtHour, archivedAt time.Time, status string, version Version, err error) {
	name, version = getVersion(name)

	if !strings.HasPrefix(name, archiveFilePrefix) {
		return storx.NodeID{}, time.Time{}, time.Time{}, "", version, Error.New("invalid path: %q", name)
	}
...
func UnsentFileName(satelliteID storx.NodeID, creationTime time.Time, version Version) string {
...
func ArchiveFileName(satelliteID storx.NodeID, creationTime, archiveTime time.Time, status pb.SettlementWithWindowResponse_Status, version Version) string {
