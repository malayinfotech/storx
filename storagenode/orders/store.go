...
import (
	...
	"common/storx"
	"storx/private/date"
	"storx/storagenode/orders/ordersfile"
)
...
func (store *FileStore) BeginEnqueue(satelliteID storx.NodeID, createdAt time.Time) (commit func(*ordersfile.Info) error, err error) {
	store.unsentMu.Lock()
...
func (store *FileStore) enqueueStartedLocked(satelliteID storx.NodeID, createdAt time.Time) {
...
// window.
func (store *FileStore) enqueueFinishedLocked(satelliteID storx.NodeID, createdAt time.Time) {
	window := activeWindow{
		satelliteID: satelliteID,
		timestamp:   date.TruncateToHourInNano(createdAt),
	}
...
func (store *FileStore) hasActiveEnqueue(satelliteID storx.NodeID, createdAt time.Time) bool {
	store.activeMu.Lock()
	defer store.activeMu.Unlock()
...
func (store *FileStore) ListUnsentBySatellite(ctx context.Context, now time.Time) (infoMap map[storx.NodeID]UnsentInfo, err error) {
	...
	var errList error
	infoMap = make(map[storx.NodeID]UnsentInfo)
...
func (store *FileStore) Archive(satelliteID storx.NodeID, unsentInfo UnsentInfo, archivedAt time.Time, status pb.SettlementWithWindowResponse_Status) error {
	store.unsentMu.Lock()
	defer store.unsentMu.Unlock()
	store.archiveMu.Lock()
	defer store.archiveMu.Unlock()
