...
type DB interface {
	...
	GetDaily(ctx context.Context, satelliteID storx.NodeID, from, to time.Time) ([]Stamp, error)
	...
	SatelliteSummary(ctx context.Context, satelliteID storx.NodeID, from, to time.Time) (float64, float64, error)
}

// Stamp is storage usage stamp for satellite from interval start till next interval.
type Stamp struct {
	SatelliteID storx.NodeID `json:"-"`
