...
func TestStorageUsage(t *testing.T) {
	...
	expectedDailyStamps := make(map[storx.NodeID]map[time.Time]storageusage.Stamp)
	expectedDailyStampsTotals := make(map[time.Time]float64)
	summaryBySatellite := make(map[storx.NodeID]float64)
	averageBySatellite := make(map[storx.NodeID]float64)
...
func TestEmptyStorageUsage(t *testing.T) {
	storagenodedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db storagenode.DB) {
		...
		t.Run("get daily", func(t *testing.T) {
			res, err := storageUsageDB.GetDaily(ctx, storx.NodeID{}, time.Time{}, now)
			assert.NoError(t, err)
			assert.Nil(t, res)
		})
		...
		t.Run("summary satellite", func(t *testing.T) {
			summ, averageUsageInBytes, err := storageUsageDB.SatelliteSummary(ctx, storx.NodeID{}, time.Time{}, now)
			assert.NoError(t, err)
