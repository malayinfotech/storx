...
func TestPieces(t *testing.T) {
	...
	satelliteID := testidentity.MustPregeneratedSignedIdentity(0, storx.LatestIDVersion()).ID
	...
	{ // write cancel
		cancelledPieceID := storx.NewPieceID()
...
func writeAPiece(ctx context.Context, t testing.TB, store *pieces.Store, satelliteID storx.NodeID, pieceID storx.PieceID, data []byte, atTime time.Time, expireTime *time.Time, formatVersion storage.FormatVersion) {
...
func tryOpeningAPiece(ctx context.Context, t testing.TB, store *pieces.Store, satelliteID storx.NodeID, pieceID storx.PieceID, expectDataLen int, expectTime time.Time, expectFormat storage.FormatVersion) {
	reader, err := store.Reader(ctx, satelliteID, pieceID)
	require.NoError(t, err)
...
func verifyPieceData(ctx context.Context, t testing.TB, store *pieces.Store, satelliteID storx.NodeID, pieceID storx.PieceID, formatVer storage.FormatVersion, expected []byte, expiration time.Time, publicKey storx.PiecePublicKey) {
