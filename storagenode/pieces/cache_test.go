...
func TestUpdate(t *testing.T) {
	...
	startSats := map[storx.NodeID]pieces.SatelliteUsage{
		sat1: {
			Total:       -20,
			ContentSize: -21,
		},
...
func TestCacheInit(t *testing.T) {
	storagenodedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db storagenode.DB) {
		...
		require.Equal(t, int64(0), piecesTotal)
		require.Equal(t, int64(0), piecesContentSize)
		satPiecesTotal, satPiecesContentSize, err := cache.SpaceUsedBySatellite(ctx, storx.NodeID{1})
		require.NoError(t, err)
		...
		expectedTotalBySA := map[storx.NodeID]pieces.SatelliteUsage{
			{1}: {
				Total:       100,
				ContentSize: 101,
			},
		...
		sat1PiecesTotal, sat1PiecesContentSize, err := cache.SpaceUsedBySatellite(ctx, storx.NodeID{1})
		require.NoError(t, err)
		require.Equal(t, int64(100), sat1PiecesTotal)
		require.Equal(t, int64(101), sat1PiecesContentSize)
		sat2PiecesTotal, sat2PiecesContentSize, err := cache.SpaceUsedBySatellite(ctx, storx.NodeID{2})
		require.NoError(t, err)
...
func TestPersistCacheTotals(t *testing.T) {
	storagenodedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db storagenode.DB) {
		...
		var expectedTotalsBySA = map[storx.NodeID]pieces.SatelliteUsage{}
		...
		expectedTotalsBySA = map[storx.NodeID]pieces.SatelliteUsage{
			{1}: {
				Total:       100,
				ContentSize: 101,
			},
		...
		trashDelta := int64(35)
		cache.Update(ctx, storx.NodeID{1}, -piecesTotalDelta, -piecesContentSizeDelta, trashDelta)
		err = cacheService.PersistCacheTotals(ctx)
		require.NoError(t, err)
...
func TestRecalculateCache(t *testing.T) {
	...
	for _, tt := range testCases {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			ID1 := storx.NodeID{1, 1}
			cache := pieces.NewBlobsUsageCacheTest(log, nil,
				tt.piecesTotal.end,
				tt.piecesContentSize.end,
				tt.trash.end,
				map[storx.NodeID]pieces.SatelliteUsage{ID1: {Total: tt.piecesTotal.end, ContentSize: tt.piecesContentSize.end}},
			)
			...
			cache.Recalculate(
				...
				tt.trash.new,
				tt.trash.start,
				map[storx.NodeID]pieces.SatelliteUsage{ID1: {Total: tt.piecesTotal.new, ContentSize: tt.piecesContentSize.new}},
				map[storx.NodeID]pieces.SatelliteUsage{ID1: {Total: tt.piecesTotal.start, ContentSize: tt.piecesContentSize.start}},
			)
...
func TestRecalculateCacheMissed(t *testing.T) {
	...
	cache := pieces.NewBlobsUsageCacheTest(log, nil,
		...
		200,
		100,
		map[storx.NodeID]pieces.SatelliteUsage{ID1: {Total: 100, ContentSize: 50}, ID2: {Total: 100, ContentSize: 50}},
	)
	...
	cache.Recalculate(
		...
		200,
		0,
		map[storx.NodeID]pieces.SatelliteUsage{ID1: {Total: 100, ContentSize: 50}},
		map[storx.NodeID]pieces.SatelliteUsage{ID1: {Total: 0, ContentSize: 0}},
	)
...
func TestCacheCreateDeleteAndTrash(t *testing.T) {
	storagenodedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db storagenode.DB) {
		...
		assertValues := func(msg string, satID storx.NodeID, expPiecesTotal, expPiecesContentSize, expTrash int) {
			piecesTotal, piecesContentSize, err := cache.SpaceUsedForPieces(ctx)
