...
type PieceExpirationDB interface {
	...
	SetExpiration(ctx context.Context, satellite storx.NodeID, pieceID storx.PieceID, expiresAt time.Time) error
	// DeleteExpiration removes an expiration record for the given piece ID on the given satellite
	DeleteExpiration(ctx context.Context, satellite storx.NodeID, pieceID storx.PieceID) (found bool, err error)
	// DeleteFailed marks an expiration record as having experienced a failure in deleting the
	// piece from the disk
	DeleteFailed(ctx context.Context, satelliteID storx.NodeID, pieceID storx.PieceID, failedAt time.Time) error
	// Trash marks a piece as in the trash
	Trash(ctx context.Context, satelliteID storx.NodeID, pieceID storx.PieceID) error
	// RestoreTrash marks all piece as not being in trash
	RestoreTrash(ctx context.Context, satelliteID storx.NodeID) error
}
...
type V0PieceInfoDB interface {
	...
	Get(ctx context.Context, satelliteID storx.NodeID, pieceID storx.PieceID) (*Info, error)
	// Delete deletes Info about a piece.
	Delete(ctx context.Context, satelliteID storx.NodeID, pieceID storx.PieceID) error
	...
	// early.
	WalkSatelliteV0Pieces(ctx context.Context, blobStore storage.Blobs, satellite storx.NodeID, walkFunc func(StoredPieceAccess) error) error
}
...
type PieceSpaceUsedDB interface {
	...
	GetPieceTotalsForAllSatellites(ctx context.Context) (map[storx.NodeID]SatelliteUsage, error)
	// UpdatePieceTotalsForAllSatellites updates each record for total spaced used with a new value for each satelliteID
	UpdatePieceTotalsForAllSatellites(ctx context.Context, newTotalsBySatellites map[storx.NodeID]SatelliteUsage) error
...
func (store *Store) CreateVerificationFile(ctx context.Context, id storx.NodeID) error {
...
func (store *Store) VerifyStorageDir(ctx context.Context, id storx.NodeID) error {
...
func (store *Store) VerifyStorageDirWithTimeout(ctx context.Context, id storx.NodeID, timeout time.Duration) error {
...
func (store *Store) Writer(ctx context.Context, satellite storx.NodeID, pieceID storx.PieceID, hashAlgorithm pb.PieceHashAlgorithm) (_ *Writer, err error) {
...
// type).
func (store StoreForTest) WriterForFormatVersion(ctx context.Context, satellite storx.NodeID,
	pieceID storx.PieceID, formatVersion storage.FormatVersion, hashAlgorithm pb.PieceHashAlgorithm) (_ *Writer, err error) {
...
func (store *Store) Reader(ctx context.Context, satellite storx.NodeID, pieceID storx.PieceID) (_ *Reader, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (store *Store) ReaderWithStorageFormat(ctx context.Context, satellite storx.NodeID,
	pieceID storx.PieceID, formatVersion storage.FormatVersion) (_ *Reader, err error) {
...
func (store *Store) Delete(ctx context.Context, satellite storx.NodeID, pieceID storx.PieceID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (store *Store) DeleteExpired(ctx context.Context, satellite storx.NodeID, pieceID storx.PieceID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (store *Store) DeleteSatelliteBlobs(ctx context.Context, satellite storx.NodeID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
// pieceExpirationDB.
func (store *Store) Trash(ctx context.Context, satellite storx.NodeID, pieceID storx.PieceID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (store *Store) EmptyTrash(ctx context.Context, satelliteID storx.NodeID, trashedBefore time.Time) (err error) {
	defer mon.Task()(&ctx)(&err)
	...
	for _, deletedID := range deletedIDs {
		pieceID, pieceIDErr := storx.PieceIDFromBytes(deletedID)
		if pieceIDErr != nil {
...
func (store *Store) RestoreTrash(ctx context.Context, satelliteID storx.NodeID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (store *Store) MigrateV0ToV1(ctx context.Context, satelliteID storx.NodeID, pieceID storx.PieceID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (store *Store) GetHashAndLimit(ctx context.Context, satellite storx.NodeID, pieceID storx.PieceID, reader *Reader) (pb.PieceHash, pb.OrderLimit, error) {
...
func (store *Store) WalkSatellitePieces(ctx context.Context, satellite storx.NodeID, walkFunc func(StoredPieceAccess) error) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (store *Store) SetExpiration(ctx context.Context, satellite storx.NodeID, pieceID storx.PieceID, expiresAt time.Time) (err error) {
...
func (store *Store) getAllStoringSatellites(ctx context.Context) ([]storx.NodeID, error) {
	namespaces, err := store.blobs.ListNamespaces(ctx)
	if err != nil {
		return nil, err
	}
	satellites := make([]storx.NodeID, len(namespaces))
	for i, namespace := range namespaces {
		satellites[i], err = storx.NodeIDFromBytes(namespace)
		if err != nil {
			return nil, err
		}
	}
...
//
// This returns both the total size of pieces plus the contentSize of pieces.
func (store *Store) SpaceUsedBySatellite(ctx context.Context, satelliteID storx.NodeID) (piecesTotal, piecesContentSize int64, err error) {
	defer mon.Task()(&ctx)(&err)
	if cache, ok := store.blobs.(*BlobsUsageCache); ok {
...
func (store *Store) SpaceUsedTotalAndBySatellite(ctx context.Context) (piecesTotal, piecesContentSize int64, totalBySatellite map[storx.NodeID]SatelliteUsage, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	totalBySatellite = map[storx.NodeID]SatelliteUsage{}
...
func (store *Store) GetV0PieceInfo(ctx context.Context, satellite storx.NodeID, pieceID storx.PieceID) (*Info, error) {
...
func (store *Store) Stat(ctx context.Context, satellite storx.NodeID, pieceID storx.PieceID) (storage.BlobInfo, error) {
...
func newStoredPieceAccess(store *Store, blobInfo storage.BlobInfo) (storedPieceAccess, error) {
	pieceID, err := storx.PieceIDFromBytes(blobInfo.BlobRef().Key)
	if err != nil {
...
func (access storedPieceAccess) PieceID() storx.PieceID {
	return access.pieceID
}

// Satellite returns the satellite ID that owns the piece.
func (access storedPieceAccess) Satellite() (storx.NodeID, error) {
	return storx.NodeIDFromBytes(access.BlobRef().Namespace)
}
