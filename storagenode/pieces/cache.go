...
type BlobsUsageCache struct {
	...
	trashTotal           int64
	spaceUsedBySatellite map[storx.NodeID]SatelliteUsage
}
...
func NewBlobsUsageCacheTest(log *zap.Logger, blob storage.Blobs, piecesTotal, piecesContentSize, trashTotal int64, spaceUsedBySatellite map[storx.NodeID]SatelliteUsage) *BlobsUsageCache {
	return &BlobsUsageCache{
		log:                  log,
		Blobs:                blob,
...
func (blobs *BlobsUsageCache) init(pieceTotal, contentSize, trashTotal int64, totalsBySatellite map[storx.NodeID]SatelliteUsage) {
	blobs.mu.Lock()
...
func (blobs *BlobsUsageCache) SpaceUsedBySatellite(ctx context.Context, satelliteID storx.NodeID) (piecesTotal int64, piecesContentSize int64, err error) {
	blobs.mu.Lock()
	defer blobs.mu.Unlock()
...
func (blobs *BlobsUsageCache) Delete(ctx context.Context, blobRef storage.BlobRef) error {
	...
	satelliteID, err := storx.NodeIDFromBytes(blobRef.Namespace)
	if err != nil {
		return err
	}
...
func (blobs *BlobsUsageCache) Update(ctx context.Context, satelliteID storx.NodeID, piecesTotalDelta, piecesContentSizeDelta, trashDelta int64) {
	blobs.mu.Lock()
	defer blobs.mu.Unlock()
...
func (blobs *BlobsUsageCache) EmptyTrash(ctx context.Context, namespace []byte, trashedBefore time.Time) (int64, [][]byte, error) {
	satelliteID, err := storx.NodeIDFromBytes(namespace)
	if err != nil {
		return 0, nil, err
	}
...
func (blobs *BlobsUsageCache) copyCacheTotals() BlobsUsageCache {
	blobs.mu.Lock()
	defer blobs.mu.Unlock()
	var copyMap = map[storx.NodeID]SatelliteUsage{}
...
func (blobs *BlobsUsageCache) Recalculate(
	...
	totalsBySatellite,
	totalsBySatelliteAtStart map[storx.NodeID]SatelliteUsage,
) {
	...
	var estimatedTotalsBySatellite = map[storx.NodeID]SatelliteUsage{}
...
func getMissed(endTotals, newTotals map[storx.NodeID]SatelliteUsage) map[storx.NodeID]SatelliteUsage {
	var missed = map[storx.NodeID]SatelliteUsage{}
