...
func (*Status) Type() string { return "storx.Status" }
...
type Service struct {
	...
	cond    sync.Cond
	queued  map[storx.NodeID]Request
	working map[storx.NodeID]struct{}
...
func NewService(log *zap.Logger, store *pieces.Store, config Config) *Service {
	return &Service{
		...
		queued:  make(map[storx.NodeID]Request),
		working: make(map[storx.NodeID]struct{}),
...
func (s *Service) trash(ctx context.Context, satelliteID storx.NodeID, pieceID storx.PieceID) (err error) {
