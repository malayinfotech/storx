...
type DB interface {
	Add(ctx context.Context, satelliteID storx.NodeID, action pb.PieceAction, amount int64, created time.Time) error
	...
	SatelliteSummary(ctx context.Context, satelliteID storx.NodeID, from, to time.Time) (*Usage, error)
	// SatelliteEgressSummary returns egress bandwidth usage for a particular satellite.
	SatelliteEgressSummary(ctx context.Context, satelliteID storx.NodeID, from, to time.Time) (*Usage, error)
	// SatelliteIngressSummary returns ingress bandwidth usage for a particular satellite.
	SatelliteIngressSummary(ctx context.Context, satelliteID storx.NodeID, from, to time.Time) (*Usage, error)
	SummaryBySatellite(ctx context.Context, from, to time.Time) (map[storx.NodeID]*Usage, error)
	...
	GetDailySatelliteRollups(ctx context.Context, satelliteID storx.NodeID, from, to time.Time) ([]UsageRollup, error)
}
