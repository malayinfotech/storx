...
type Chore struct {
	...
	mu       sync.Mutex
	cycles   map[storx.NodeID]*sync2.Cycle
	started  sync2.Fence
...
func NewChore(log *zap.Logger, interval time.Duration, service *Service) *Chore {
	return &Chore{
		...
		cycles:   make(map[storx.NodeID]*sync2.Cycle),
		interval: interval,
	}
}
...
func (chore *Chore) updateCycles(ctx context.Context, group *errgroup.Group, satellites []storx.NodeID) {
	chore.mu.Lock()
