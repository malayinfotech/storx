...
func (db *satellitesDB) SetAddress(ctx context.Context, satelliteID storx.NodeID, address string) (err error) {
...
func (db *satellitesDB) GetSatellite(ctx context.Context, satelliteID storx.NodeID) (satellite satellites.Satellite, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (db *satellitesDB) GetSatellitesUrls(ctx context.Context) (satelliteURLs []storx.NodeURL, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	var urls []storx.NodeURL
	for rows.Next() {
		var url storx.NodeURL
...
func (db *satellitesDB) InitiateGracefulExit(ctx context.Context, satelliteID storx.NodeID, intitiatedAt time.Time, startingDiskUsage int64) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (db *satellitesDB) CancelGracefulExit(ctx context.Context, satelliteID storx.NodeID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (db *satellitesDB) UpdateGracefulExit(ctx context.Context, satelliteID storx.NodeID, addToBytesDeleted int64) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (db *satellitesDB) CompleteGracefulExit(ctx context.Context, satelliteID storx.NodeID, finishedAt time.Time, exitStatus satellites.Status, completionReceipt []byte) (err error) {
	defer mon.Task()(&ctx)(&err)
	return ErrSatellitesDB.Wrap(withTx(ctx, db.GetDB(), func(tx tagsql.Tx) error {
