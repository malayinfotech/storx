...
const (
	...
	FullSize = memory.Size(len(storx.SerialNumber{}))
)
...
type serialsList struct {
	...
	fullSerials    []storx.SerialNumber
}
...
type Table struct {
	...
	serials map[storx.NodeID]map[int64]serialsList
...
func NewTable(maxMemory memory.Size) *Table {
	...
	return &Table{
		serials:   make(map[storx.NodeID]map[int64]serialsList),
		maxMemory: maxMemory,
	}
}
...
func (table *Table) Add(satelliteID storx.NodeID, serialNumber storx.SerialNumber, expiration time.Time) error {
	table.mu.Lock()
...
func (table *Table) Exists(satelliteID storx.NodeID, serialNumber storx.SerialNumber, expiration time.Time) bool {
	table.mu.Lock()
	defer table.mu.Unlock()
...
// or returns an error if it is already in the list.
func insertSerial(list []storx.SerialNumber, serial storx.SerialNumber) ([]storx.SerialNumber, error) {
	i := sort.Search(len(list), func(h int) bool {
		return serial.Less(list[h])
	})
	// if serial is already in the list, it will be at index i-1
	...
	// insert new serial at index i and shift everything up
	// 1. grow the slice by one element.
	list = append(list, storx.SerialNumber{})
	// 2. move the upper part of the slice out of the way and open a hole.
	copy(list[i+1:], list[i:])
	// 3. store the new value.
	list[i] = serial
...
func tryTruncate(serial storx.SerialNumber, expiration time.Time) (partial Partial, succeeded bool) {
