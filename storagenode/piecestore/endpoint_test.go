...
func TestDeletePieces(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		t.Run("ok", func(t *testing.T) {
			pieceIDs := []storx.PieceID{testrand.PieceID(), testrand.PieceID(), testrand.PieceID(), testrand.PieceID()}
		...
		t.Run("error: permission denied", func(t *testing.T) {
			...
			_, err = client.DeletePieces(ctx.Context, &pb.DeletePiecesRequest{
				PieceIds: []storx.PieceID{pieceID},
			})
			require.Error(t, err)
	...
	testplanet.Run(t, testplanet.Config{
		...
		for i, uplink := range planet.Uplinks {
			...
			ctx.Go(func() (err error) {
				...
				pieceID := storx.PieceID{byte(i + 1)}
				serialNumber := testrand.SerialNumber()
...
func uploadPiece(
	t *testing.T, ctx *testcontext.Context, piece storx.PieceID, storageNode *testplanet.StorageNode,
...
func downloadPiece(
	t *testing.T, ctx *testcontext.Context, piece storx.PieceID, limit int64,
...
func TestExists(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		existingSNPieces := map[storx.NodeID][]storx.PieceID{}
