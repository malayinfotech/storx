...
func TestListAgainstSpec(t *testing.T) {
	...
	makeNodeURL := func(s string) storx.NodeURL {
		u, err := storx.ParseNodeURL(fixURL(s))
		require.NoError(t, err)
		return u
	}
...
func makeTestID(x byte) storx.NodeID {
	var id storx.NodeID
	copy(id[:], bytes.Repeat([]byte{x}, len(id)))
	return storx.NewVersionedID(id, storx.IDVersions[storx.V0])
}
