...
func TestSatelliteURLNodeURLConversion(t *testing.T) {
	nodeURL, err := storx.ParseNodeURL("121RTSDpyNZVcEU84Ticf2L1ntiuUimbWgfATz21tuvgk3vzoA6@127.0.0.1:7777")
	require.NoError(t, err)
...
func TestParseSatelliteURL_Valid(t *testing.T) {
	id, err := storx.NodeIDFromString("121RTSDpyNZVcEU84Ticf2L1ntiuUimbWgfATz21tuvgk3vzoA6")
	require.NoError(t, err)
	...
	for _, tt := range []struct {
		...
		{
			name: "success with storx schema",
			url:  "storx://121RTSDpyNZVcEU84Ticf2L1ntiuUimbWgfATz21tuvgk3vzoA6@127.0.0.1:7777/",
			parsed: trust.SatelliteURL{
				ID:   id,
				Host: "127.0.0.1",
				Port: 7777,
