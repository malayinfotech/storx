...
type IdentityResolver interface {
	...
	ResolveIdentity(ctx context.Context, url storx.NodeURL) (*identity.PeerIdentity, error)
}

// IdentityResolverFunc is a convenience type for implementing IdentityResolver using a
// function literal.
type IdentityResolverFunc func(ctx context.Context, url storx.NodeURL) (*identity.PeerIdentity, error)

// ResolveIdentity returns the peer identity of the peer located at the Node URL.
func (fn IdentityResolverFunc) ResolveIdentity(ctx context.Context, url storx.NodeURL) (*identity.PeerIdentity, error) {
	return fn(ctx, url)
}

// Dialer implements an IdentityResolver using an RPC dialer.
func Dialer(dialer rpc.Dialer) IdentityResolver {
	return IdentityResolverFunc(func(ctx context.Context, url storx.NodeURL) (_ *identity.PeerIdentity, err error) {
...
type Pool struct {
	...
	satellites   map[storx.NodeID]*satelliteInfoCache
}
...
func NewPool(log *zap.Logger, resolver IdentityResolver, config Config, satellitesDB satellites.DB) (*Pool, error) {
	...
	return &Pool{
		...
		satellites:      make(map[storx.NodeID]*satelliteInfoCache),
	}, nil
}
...
func (pool *Pool) VerifySatelliteID(ctx context.Context, id storx.NodeID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (pool *Pool) GetSignee(ctx context.Context, id storx.NodeID) (_ signing.Signee, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (pool *Pool) GetSatellites(ctx context.Context) (satellites []storx.NodeID) {
	defer mon.Task()(&ctx)(nil)
	for sat := range pool.satellites {
		satellites = append(satellites, sat)
	}
	sort.Sort(storx.NodeIDList(satellites))
	return satellites
}

// GetNodeURL returns the node url of a satellite in the trusted list.
func (pool *Pool) GetNodeURL(ctx context.Context, id storx.NodeID) (_ storx.NodeURL, err error) {
	defer mon.Task()(&ctx)(&err)

	info, err := pool.getInfo(id)
	if err != nil {
		return storx.NodeURL{}, err
	}
	return info.url, nil
}
...
func (pool *Pool) Refresh(ctx context.Context) error {
	...
	trustedIDs := make(map[storx.NodeID]struct{})
...
func (pool *Pool) getInfo(id storx.NodeID) (*satelliteInfoCache, error) {
...
func (pool *Pool) fetchURLs(ctx context.Context) ([]storx.NodeURL, error) {
