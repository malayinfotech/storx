...
basepath=$HOME/.storx/local-network
...
case $1 in
	...
	;;
	setup)
		temp_build "storx-sim" identity
		echo "setting up storx-sim"
		${storx_sim} network setup
	...
	alpha)
		build "storx-sim"
		${storx_sim} network run --config ${alpha_config}
	;;
	unauthorized)
		build "storx-sim"
		${storx_sim} network run --config ${unauthorized_config}
	;;
	run)
		${storx_sim} network run
	;;
	*)
		$@ --help
	;;
