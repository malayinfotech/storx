...
github-release release --user storx --repo storx --tag "$TAG" --description "$changelog" --draft
...
for app in $apps; do
  for file in "$FOLDER/$app"*.zip; do
    github-release upload --user storx --repo storx --tag "$TAG" --name $(basename "$file") --file "$file"
  done
done
