...
: "${STORX_NETWORK_DIR?Environment variable STORX_NETWORK_DIR needs to be set}"
...
PRISTINE_FILES_DIR="$STORX_NETWORK_DIR/pristine/$BUCKET"
DOWNLOAD_FILES_DIR="$STORX_NETWORK_DIR/download/$BUCKET"

# override configured access with access where address is node ID + satellite addess
STORX_ACCESS=$(go run "$SCRIPTDIR"/../../update-access.go "$SATELLITE_0_DIR" "$GATEWAY_0_ACCESS")
UPLINK_ACCESS="$STORX_ACCESS"
...
if [[ "$1" == "cleanup" ]]; then
    for BUCKET_DIR in "$STORX_NETWORK_DIR"/pristine/*; do
