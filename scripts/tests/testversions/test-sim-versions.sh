...
install_sim(){
    ...
    if [ -d "${work_dir}/cmd/gateway" ]; then
        (cd ${work_dir}/cmd/gateway && go build -race -v -o ${bin_dir}/gateway storx/storx/cmd/gateway >/dev/null 2>&1)
    else
        GOBIN=${bin_dir} go install -race storx/gateway@latest
    fi
    if [ -d "${work_dir}/cmd/multinode" ]; then
        # as storx-sim is most likely installed from $PWD and contains storx-sim version which requires multinode
...
setup_stage(){
    ...
    ln -f $(version_dir ${sat_version})/bin/storx-sim $test_dir/bin/storx-sim
    ...
    (
        ...
        then
            ...
            if version_ge "$version" "v1.64.0"; then
                ...
                go1.16.15 build -race -v -o ${bin_dir}/uplink storx/storx/cmd/uplink >/dev/null 2>&1
            fi
...
for ul_version in ${stage1_uplink_versions}; do
    ...
    PATH=$test_dir/bin:$PATH storx-sim -x --host "${STORX_NETWORK_HOST4}" --config-dir "${test_dir}/local-network" network test bash "${scriptdir}/test-versions.sh" "${test_dir}/local-network" "upload" "${ul_version}" "$update_access_script_path"
done
...
for ul_version in ${stage2_uplink_versions}; do
    ...
    src_ul_version_dir=$(version_dir ${ul_version})
    ln -f ${src_ul_version_dir}/bin/uplink $test_dir/bin/uplink
    PATH=$test_dir/bin:$PATH storx-sim -x --host "${STORX_NETWORK_HOST4}" --config-dir "${test_dir}/local-network" network test bash "${scriptdir}/test-versions.sh" "${test_dir}/local-network" "download" "${ul_version}" "$update_access_script_path" "${stage1_uplink_versions}"
done


echo -e "\nCleaning up."
PATH=$test_dir/bin:$PATH storx-sim -x --host "${STORX_NETWORK_HOST4}" --config-dir "${test_dir}/local-network" network test bash "${scriptdir}/test-versions.sh" "${test_dir}/local-network" "cleanup" "${stage1_uplink_versions}" ""
