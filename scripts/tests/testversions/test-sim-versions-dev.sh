...
export STORX_SIM_POSTGRES="postgres://postgres@localhost:5433/teststorx?sslmode=disable"
export STORX_SIM_REDIS="localhost:6380"
...
docker exec postgres-$BUILD_NUMBER createdb -U postgres teststorx
# fetch the remote main branch
git fetch --no-tags --progress -- https://github.com/storx/storx.git +refs/heads/main:refs/remotes/origin/main
