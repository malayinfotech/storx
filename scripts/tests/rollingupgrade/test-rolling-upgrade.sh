...
# This script assumes that storx-sim and uplink has already been set up and initial files have been
...
echo "Begin test-rolling-upgrade.sh, storx-sim config directory:" ${main_cfg_dir}

echo "which storx-sim: $(which storx-sim)"
echo "Shasum for storx-sim:"
shasum $(which storx-sim)

if [ ! -d ${main_cfg_dir}/uplink-old-api ]; then
    mkdir -p ${main_cfg_dir}/uplink-old-api
    access=$(storx-sim --config-dir=$main_cfg_dir network env GATEWAY_0_ACCESS)
    new_access=$(go run $update_access_script_path $(storx-sim --config-dir=$main_cfg_dir network env SATELLITE_0_DIR) $access)
