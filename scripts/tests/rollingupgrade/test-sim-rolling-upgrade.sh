...
# Description of test:
# Stage 1:
#  * Set up a storx-sim network on the latest point release (all storagenodes and satellite on latest point release).
...
install_sim(){
    ...
    go build -race -v -o ${bin_dir}/storagenode storx/storx/cmd/storagenode >/dev/null 2>&1
    go build -race -v -o ${bin_dir}/satellite storx/storx/cmd/satellite >/dev/null 2>&1
    go build -race -v -o ${bin_dir}/storx-sim storx/storx/cmd/storx-sim >/dev/null 2>&1
    go build -race -v -o ${bin_dir}/versioncontrol storx/storx/cmd/versioncontrol >/dev/null 2>&1

    go build -race -v -o ${bin_dir}/uplink storx/storx/cmd/uplink >/dev/null 2>&1
    go build -race -v -o ${bin_dir}/identity storx/storx/cmd/identity >/dev/null 2>&1
    go build -race -v -o ${bin_dir}/certificates storx/storx/cmd/certificates >/dev/null 2>&1

    if [ -d "${work_dir}/cmd/gateway" ]; then
        pushd ${work_dir}/cmd/gateway
            go build -race -v -o ${bin_dir}/gateway storx/storx/cmd/gateway >/dev/null 2>&1
        popd
    else
        GOBIN=${bin_dir} go install -race storx/gateway@latest
    fi

    if [ -d "${work_dir}/cmd/multinode" ]; then
        go build -race -v -o ${bin_dir}/multinode storx/storx/cmd/multinode >/dev/null 2>&1
    fi
}
...
setup_stage(){
    ...
    PATH=$src_sat_version_dir/bin:$PATH src_sat_cfg_dir=$(storx-sim network env --config-dir=${src_sat_version_dir}/local-network/ SATELLITE_0_DIR)
    PATH=$test_dir/bin:$PATH dest_sat_cfg_dir=$(storx-sim network env --config-dir=${test_dir}/local-network/ SATELLITE_0_DIR)
    PATH=$src_sat_version_dir/bin:$PATH src_mnd_cfg_dir=$(storx-sim network env --config-dir=${src_sat_version_dir}/local-network/ MULTINODE_0_DIR)
    PATH=$src_sat_version_dir/bin:$PATH dst_mnd_cfg_dir=$(storx-sim network env --config-dir=${test_dir}/local-network/ MULTINODE_0_DIR)
    ...
    ln -f $src_sat_version_dir/bin/storx-sim $test_dir/bin/storx-sim
    ...
    for sn_version in ${stage_sn_versions}; do
        ...
        PATH=$src_sn_version_dir/bin:$PATH src_sn_cfg_dir=$(storx-sim network env --config-dir=${src_sn_version_dir}/local-network/ STORAGENODE_${counter}_DIR)
        PATH=$test_dir/bin:$PATH dest_sn_cfg_dir=$(storx-sim network env --config-dir=${test_dir}/local-network/ STORAGENODE_${counter}_DIR)
...
STORX_SIM_REDIS=${STORX_SIM_REDIS:-""}

if [ -z ${STORX_SIM_POSTGRES} ]; then
    echo "STORX_SIM_POSTGRES is required for the satellite DB. Example: STORX_SIM_POSTGRES=postgres://[user]:[pass]@[host]/[db]?sslmode=disable"
    exit 1
fi

if [ -z ${STORX_SIM_REDIS} ]; then
    echo "STORX_SIM_REDIS is required for the satellite DB. Example: STORX_SIM_REDIS=127.0.0.1:[port]"
    exit 1
fi
...
for version in ${unique_versions}; do
    ...
    then
        echo "Installing storx-sim for ${version} in ${dir}."
        pushd ${dir}
        ...
        popd
        echo "Setting up storx-sim for ${version}. Bin: ${bin_dir}, Config: ${dir}/local-network"
        PATH=${bin_dir}:$PATH storx-sim -x --host="${STORX_NETWORK_HOST4}" --postgres="${STORX_SIM_POSTGRES}" --config-dir "${dir}/local-network" network setup > /dev/null 2>&1
        echo "Finished setting up. ${dir}/local-network:" $(ls ${dir}/local-network)
        echo "Binary shasums:"
        shasum ${bin_dir}/storx-sim
...
PATH=$test_dir/bin:$PATH storx-sim -x --host "${STORX_NETWORK_HOST4}" --config-dir "${test_dir}/local-network" network test bash "${test_versions_path}" "${test_dir}/local-network" "upload" "${stage1_uplink_version}" "$update_access_script_path"
...
storx-sim tool wait-for --retry 50 --interval 100ms  "127.0.0.1:30000"
...
for ul_version in ${stage2_uplink_versions}; do
    ...
    ln -f ${src_ul_version_dir}/bin/uplink $test_dir/bin/uplink
    PATH=$test_dir/bin:$PATH storx-sim -x --host "${STORX_NETWORK_HOST4}" --config-dir "${test_dir}/local-network" network test bash "${scriptdir}/test-rolling-upgrade.sh" "${test_dir}/local-network"  "${stage1_uplink_version}" "$update_access_script_path"

    if [[ $ul_version == $current_commit ]];then
        echo "Running final upload/download test on $current_commit"
        PATH=$test_dir/bin:$PATH storx-sim -x --host "${STORX_NETWORK_HOST4}" --config-dir "${test_dir}/local-network" network test bash "${scriptdir}/test-rolling-upgrade-final-upload.sh" "${test_dir}/local-network"
    fi
done

echo -e "\nCleaning up."
PATH=$test_dir/bin:$PATH storx-sim -x --host "${STORX_NETWORK_HOST4}" --config-dir "${test_dir}/local-network" network test bash "${test_versions_path}" "${test_dir}/local-network" "cleanup" "${stage1_uplink_version}" ""
