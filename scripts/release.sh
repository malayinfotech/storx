...
echo Running "go $@"
exec go "$1" -ldflags \
  "-X storx/private/version.buildTimestamp=$TIMESTAMP
   -X storx/private/version.buildCommitHash=$COMMIT
   -X storx/private/version.buildVersion=$VERSION
   -X storx/private/version.buildRelease=$RELEASE" "${@:2}"
