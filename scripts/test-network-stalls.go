...
func storeFileAndCheck(uplink *uplinkRunner, srcFile, dstFile string) error {
	if _, err := uplink.Run(nil, "cp", srcFile, dstFile); err != nil {
		return errs.New("Could not copy file into storx-sim network: %v", err)
	}
	if exists, err := uplink.doesRemoteExist(dstFile); err != nil {
		return errs.New("Could not check if file exists: %v", err)
	} else if !exists {
		return errs.New("Copied file not present in storx-sim network!")
	}
	return nil
}
...
func deleteWhileStallingAndCheck(uplink *uplinkRunner, dstFile string, nodeProc *os.Process) error {
	...
	if err != nil {
		...
		return errs.New("Unexpected error trying to delete file %q from storx-sim network: %v", dstFile, err)
	}
	if exists, err := uplink.doesRemoteExist(dstFile); err != nil {
		return errs.New("Failed to check if remote file %q was deleted: %v", dstFile, err)
	} else if exists {
		return errs.New("Deleted file still present in storx-sim network!")
	}
...
func runTest() error {
	...
	if configDir == "" {
		return errs.New("This test should be run under storx-sim test ($GATEWAY_0_DIR not found).")
	}
	...
	tempDir, err := os.MkdirTemp("", "storx-test-network-stalls.")
	if err != nil {
		return err
	}
	bucket := "sj://" + *bucketName
	srcFile := filepath.Join(tempDir, "to-storx-sim")
	dstFile := bucket + "/in-storx-sim"
