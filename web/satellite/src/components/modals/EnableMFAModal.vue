...
export default class EnableMFAModal extends Vue {
    public readonly qrLink =
        `otpauth://totp/${encodeURIComponent(this.$store.getters.user.email)}?secret=${this.userMFASecret}&issuer=${encodeURIComponent(`STORX ${this.satellite}`)}&algorithm=SHA1&digits=6&period=30`;
    public isScan = true;
