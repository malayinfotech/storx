...
func (ec *ECRepairer) dialPiecestore(ctx context.Context, n storx.NodeURL) (*piecestore.Client, error) {
...
func (ec *ECRepairer) Get(ctx context.Context, limits []*pb.AddressedOrderLimit, cachedNodesInfo map[storx.NodeID]overlay.NodeReputation, privateKey storx.PiecePrivateKey, es eestream.ErasureScheme, dataSize int64, minFailures int) (_ io.ReadCloser, _ FetchResultReport, err error) {
...
func (ec *ECRepairer) downloadAndVerifyPiece(ctx context.Context, limit *pb.AddressedOrderLimit, address string, privateKey storx.PiecePrivateKey, tmpDir string, pieceSize int64) (pieceReadCloser io.ReadCloser, hash *pb.PieceHash, originalLimit *pb.OrderLimit, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	ps, err := ec.dialPiecestore(downloadCtx, storx.NodeURL{
...
func (ec *ECRepairer) Repair(ctx context.Context, limits []*pb.AddressedOrderLimit, privateKey storx.PiecePrivateKey, rs eestream.RedundancyStrategy, data io.Reader, timeout time.Duration, successfulNeeded int) (successfulNodes []*pb.Node, successfulHashes []*pb.PieceHash, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (ec *ECRepairer) putPiece(ctx, parent context.Context, limit *pb.AddressedOrderLimit, privateKey storx.PiecePrivateKey, data io.ReadCloser) (hash *pb.PieceHash, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	ps, err := ec.dialPiecestore(ctx, storx.NodeURL{
		ID:      storageNodeID,
...
func unique(limits []*pb.AddressedOrderLimit) bool {
	...
	ids := make(storx.NodeIDList, len(limits))
	for i, addressedLimit := range limits {
	...
	for i := 1; i < len(ids); i++ {
		if ids[i] != (storx.NodeID{}) && ids[i] == ids[i-1] {
			return false
		}
	}
