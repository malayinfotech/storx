...
func testDataRepair(t *testing.T, inMemoryRepair bool, hashAlgo pb.PieceHashAlgorithm) {
	...
	testplanet.Run(t, testplanet.Config{
		...
		nodesToKill := make(map[storx.NodeID]bool)
		nodesToDisqualify := make(map[storx.NodeID]bool)
		nodesToKeepAlive := make(map[storx.NodeID]bool)
...

		{
			...
			repairSettled := make(map[storx.NodeID]uint64)
...
func TestMinRequiredDataRepair(t *testing.T) {
	...
	testplanet.Run(t, testplanet.Config{
		...
		// kill nodes and track lost pieces
		var availableNodes storx.NodeIDList
		var killedNodes storx.NodeIDList
		...
		nodesReputation := make(map[storx.NodeID]reputation.Info)
...
func TestFailedDataRepair(t *testing.T) {
	...
	testplanet.Run(t, testplanet.Config{
		...
		var originalNodes storx.NodeIDList
		...
		nodesReputationAfter := make(map[storx.NodeID]reputation.Info)
		for _, piece := range availablePieces {
			info, err := reputationService.Get(ctx, piece.StorageNode)
			require.NoError(t, err)
...
func TestMissingPieceDataRepair(t *testing.T) {
	...
	testplanet.Run(t, testplanet.Config{
		...
		// kill nodes and track lost pieces
		originalNodes := make(map[storx.NodeID]bool)
		var availablePieces metabase.Pieces
...
func TestRepairExpiredSegment(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		// kill nodes and track lost pieces
		nodesToDQ := make(map[storx.NodeID]bool)
...
func TestRepairMultipleDisqualifiedAndSuspended(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		nodesToDisqualify := make(map[storx.NodeID]bool)
		nodesToSuspend := make(map[storx.NodeID]bool)
		nodesToKeepAlive := make(map[storx.NodeID]bool)
...
func TestDataRepairUploadLimit(t *testing.T) {
	...
	testplanet.Run(t, testplanet.Config{
		...
		originalStorageNodes := make(map[storx.NodeID]struct{})
		for _, p := range originalPieces {
			originalStorageNodes[p.StorageNode] = struct{}{}
		}

		killedNodes := make(map[storx.NodeID]struct{})
...
func TestRepairGracefullyExited(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		nodesToExit := make(map[storx.NodeID]bool)
		nodesToKeepAlive := make(map[storx.NodeID]bool)
...
func corruptPieceData(ctx context.Context, t *testing.T, planet *testplanet.Planet, corruptedNode *testplanet.StorageNode, corruptedPieceID storx.PieceID) {
	t.Helper()
...
func TestECRepairerGet(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		redundancy, err := eestream.NewRedundancyStrategyFromStorx(segment.Redundancy)
		require.NoError(t, err)
...
func TestSegmentInExcludedCountriesRepair(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		nodesInPointer := make(map[storx.NodeID]bool)
