...
type Dialer struct {
	...
	mu         sync.RWMutex
	dialFailed map[storx.NodeID]time.Time
}
...
func (dialer *Dialer) Handle(ctx context.Context, node storx.NodeURL, queue Queue) {
...
// for some time.
func (dialer *Dialer) markFailed(ctx context.Context, node storx.NodeURL) {
	dialer.mu.Lock()
...
func (dialer *Dialer) recentlyFailed(ctx context.Context, node storx.NodeURL) bool {
	dialer.mu.RLock()
	lastFailed, ok := dialer.dialFailed[node.ID]
	dialer.mu.RUnlock()
...
func batchJobs(jobs []Job, maxBatchSize int) (pieces []storx.PieceID, promises []Promise, rest []Job) {
...
func dialPieceStore(ctx context.Context, dialer rpc.Dialer, target storx.NodeURL) (pb.DRPCPiecestoreClient, *rpc.Conn, error) {
