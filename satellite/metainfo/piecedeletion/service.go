...
type Nodes interface {
	GetNodes(ctx context.Context, nodes []storx.NodeID) (_ map[storx.NodeID]*overlay.SelectedNode, err error)
}
...
func (service *Service) DeleteWithCustomThreshold(ctx context.Context, requests []Request, successThreshold float64) (err error) {
	...
	// request.
	nodesReqs := make(map[storx.NodeID]Request, len(requests))
	nodeIDs := []storx.NodeID{}
