...
func TestEndpoint_Object_With_StorageNodes(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		t.Run("begin commit", func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, int32(storx.EU), streamID.Placement)
			...
			fullIDMap := make(map[storx.NodeID]*identity.FullIdentity)
		...
		t.Run("upload with placement", func(t *testing.T) {
			...
			require.Equal(t, storx.EU, segments[0].Placement)
		})
	})
}
...
func TestMoveObject_Geofencing(t *testing.T) {
	testplanet.Run(t,
		...
		func(t *testing.T, ctx *testcontext.Context, planet *testplanet.Planet) {
			...
			createGeofencedBucket(t, ctx, buckets, projectID, "global1", storx.EveryCountry)
			createGeofencedBucket(t, ctx, buckets, projectID, "global2", storx.EveryCountry)
			createGeofencedBucket(t, ctx, buckets, projectID, "us1", storx.US)
			createGeofencedBucket(t, ctx, buckets, projectID, "us2", storx.US)
			createGeofencedBucket(t, ctx, buckets, projectID, "eu1", storx.EU)
...
func createGeofencedBucket(t *testing.T, ctx *testcontext.Context, buckets *buckets.Service, projectID uuid.UUID, bucketName string, placement storx.PlacementConstraint) {
...
func TestEndpoint_Object_MultipleVersions(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		t.Run("override object", func(t *testing.T) {
			...
			pieceIDs := map[storx.NodeID]storx.PieceID{}
...
func TestListUploads(t *testing.T) {
	...
	testplanet.Run(t, testplanet.Config{
		...
		// basic ListUploads tests, more tests are on storx/uplink side
		u := planet.Uplinks[0]
		s := planet.Satellites[0]
