...
func (endpoint *Endpoint) validateBucket(ctx context.Context, bucket []byte) (err error) {
	...
	if len(bucket) == 0 {
		return Error.Wrap(storx.ErrNoBucket.New(""))
	}
...
func (endpoint *Endpoint) validateRemoteSegment(ctx context.Context, commitRequest metabase.CommitSegment, originalLimits []*pb.OrderLimit) (err error) {
	...
	maxAllowed, err := encryption.CalcEncryptedSize(endpoint.config.MaxSegmentSize.Int64(), storx.EncryptionParameters{
	...
	nodeIds := make(map[storx.NodeID]struct{})
...
// is serialized to storx.Nonce automatically.
