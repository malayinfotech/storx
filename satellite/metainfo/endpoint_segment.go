...
func calculateSpaceUsed(segmentSize int64, numberOfPieces int, rs storx.RedundancyScheme) (totalStored int64) {
...
func (endpoint *Endpoint) BeginSegment(ctx context.Context, req *pb.SegmentBeginRequest) (resp *pb.SegmentBeginResponse, err error) {
	...
	defaultRedundancy := storx.RedundancyScheme{
	...
	request := overlay.FindStorageNodesRequest{
		...
		Placement:      storx.PlacementConstraint(streamID.Placement),
	}
...
func (endpoint *Endpoint) RetryBeginSegmentPieces(ctx context.Context, req *pb.RetryBeginSegmentPiecesRequest) (resp *pb.RetryBeginSegmentPiecesResponse, err error) {
	...
	excludedIDs := make([]storx.NodeID, 0, len(segmentID.OriginalOrderLimits))
	...
	nodes, err := endpoint.overlay.FindStorageNodesForUpload(ctx, overlay.FindStorageNodesRequest{
		...
		Placement:      storx.PlacementConstraint(segmentID.StreamId.Placement),
...
func (endpoint *Endpoint) CommitSegment(ctx context.Context, req *pb.SegmentCommitRequest) (resp *pb.SegmentCommitResponse, err error) {
	...
	rs := storx.RedundancyScheme{
		Algorithm:      storx.RedundancyAlgorithm(endpoint.defaultRS.Type),
...
func convertStreamListResults(result metabase.ListStreamPositionsResult) (*pb.SegmentListResponse, error) {
	...
	for i, item := range result.Segments {
		...
		var err error
		items[i].EncryptedKeyNonce, err = storx.NonceFromBytes(item.EncryptedKeyNonce)
		if err != nil {
			return nil, err
		}
