...
func (endpoint *Endpoint) BeginObject(ctx context.Context, req *pb.ObjectBeginRequest) (resp *pb.ObjectBeginResponse, err error) {
	...
	if !endpoint.config.MultipleVersions {
		if canDelete {
			...
			if err != nil && !storx.ErrObjectNotFound.Has(err) {
				return nil, err
			}
		} else {
	...
	encryptionParameters := storx.EncryptionParameters{
		CipherSuite: storx.CipherSuite(req.EncryptionParameters.CipherSuite),
...
func (endpoint *Endpoint) CommitObject(ctx context.Context, req *pb.ObjectCommitRequest) (resp *pb.ObjectCommitResponse, err error) {
	...
	encryption := storx.EncryptionParameters{}
	err = pb.Unmarshal(req.EncryptedMetadata, streamMeta)
	if err == nil {
		encryption.CipherSuite = storx.CipherSuite(streamMeta.EncryptionType)
...
func (endpoint *Endpoint) DownloadObject(ctx context.Context, req *pb.ObjectDownloadRequest) (resp *pb.ObjectDownloadResponse, err error) {
	...
	downloadSegments, err := func() ([]*pb.SegmentDownloadResponse, error) {
		...
		encryptedKeyNonce, err := storx.NonceFromBytes(segment.EncryptedKeyNonce)
		if err != nil {
...
func (endpoint *Endpoint) calculateDownloadSizes(streamRange *metabase.StreamRange, segment metabase.Segment, encryptionParams storx.EncryptionParameters) downloadSizes {
	if segment.Inline() {
	...
	enc, err := encryption.NewEncrypter(encryptionParams.CipherSuite, &storx.Key{1}, &storx.Nonce{1}, int(encryptionParams.BlockSize))
	if err != nil {
...
func (endpoint *Endpoint) GetObjectIPs(ctx context.Context, req *pb.ObjectGetIPsRequest) (resp *pb.ObjectGetIPsResponse, err error) {
	...
	nodeIDs := make([]storx.NodeID, 0, len(pieceCountByNodeID))
...
func (endpoint *Endpoint) objectToProto(ctx context.Context, object metabase.Object, rs *pb.RedundancyScheme) (*pb.Object, error) {
	...
	if len(object.EncryptedMetadataNonce) > 0 {
		nonce, err = storx.NonceFromBytes(object.EncryptedMetadataNonce)
		if err != nil {
			return nil, err
		}
	}
...
func (endpoint *Endpoint) objectEntryToProtoListItem(ctx context.Context, bucket []byte,
	...
	includeSystem, includeMetadata bool, placement storx.PlacementConstraint) (item *pb.ObjectListItem, err error) {
	...
	if includeMetadata {
		...
		if len(entry.EncryptedMetadataNonce) > 0 {
			nonce, err = storx.NonceFromBytes(entry.EncryptedMetadataNonce)
			if err != nil {
				return nil, err
			}
		}
		...
		if entry.Encryption != (storx.EncryptionParameters{}) {
...
func groupPiecesByNodeID(segments []metabase.DeletedSegmentInfo) map[storx.NodeID][]storx.PieceID {
	piecesToDelete := map[storx.NodeID][]storx.PieceID{}
...
func convertBeginMoveObjectResults(result metabase.BeginMoveObjectResult) (*pb.ObjectBeginMoveResponse, error) {
	...
	for i, key := range result.EncryptedKeysNonces {
		...
		if len(key.EncryptedKeyNonce) != 0 {
			nonce, err = storx.NonceFromBytes(key.EncryptedKeyNonce)
			if err != nil {
				return nil, err
			}
		}
	...
	var metadataNonce storx.Nonce
	var err error
	if len(result.EncryptedMetadataKeyNonce) != 0 {
		metadataNonce, err = storx.NonceFromBytes(result.EncryptedMetadataKeyNonce)
		if err != nil {
			return nil, err
		}
	}
