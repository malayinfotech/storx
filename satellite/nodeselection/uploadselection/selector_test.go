...
import (
	...
	"common/testcontext"
	"common/testrand"
	"storx/satellite/nodeselection/uploadselection"
)
...
func TestSelectByID(t *testing.T) {
	...
	var selectedNodeCount = map[storx.NodeID]int{}
...
func TestSelectFiltered(t *testing.T) {
	...
	assert.Len(t, selector.Select(3, uploadselection.Criteria{ExcludeNodeIDs: []storx.NodeID{firstID}}), 2)
	assert.Len(t, selector.Select(3, uploadselection.Criteria{}), 3)

	assert.Len(t, selector.Select(3, uploadselection.Criteria{ExcludeNodeIDs: []storx.NodeID{firstID, secondID}}), 1)
	assert.Len(t, selector.Select(3, uploadselection.Criteria{
		AutoExcludeSubnets: map[string]struct{}{},
	}), 2)
	assert.Len(t, selector.Select(3, uploadselection.Criteria{
		ExcludeNodeIDs:     []storx.NodeID{thirdID},
		AutoExcludeSubnets: map[string]struct{}{},
	}), 1)
}
