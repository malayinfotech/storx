...
func NewCachingDB(log *zap.Logger, backingStore DB, reputationConfig Config) *CachingDB {
	...
	return &CachingDB{
		...
		pending:            make(map[storx.NodeID]*cachedNodeReputationInfo),
	}
}
...
type CachingDB struct {
	...
	// from the cache.
	pending        map[storx.NodeID]*cachedNodeReputationInfo
...
// responsible for updating the records in the overlay to match.
func (cdb *CachingDB) ApplyUpdates(ctx context.Context, nodeID storx.NodeID, updates Mutations, config Config, now time.Time) (info *Info, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cdb *CachingDB) UnsuspendNodeUnknownAudit(ctx context.Context, nodeID storx.NodeID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cdb *CachingDB) DisqualifyNode(ctx context.Context, nodeID storx.NodeID, disqualifiedAt time.Time, reason overlay.DisqualificationReason) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cdb *CachingDB) SuspendNodeUnknownAudit(ctx context.Context, nodeID storx.NodeID, suspendedAt time.Time) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cdb *CachingDB) RequestSync(ctx context.Context, nodeID storx.NodeID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cdb *CachingDB) getExistingEntry(nodeID storx.NodeID, f func(entryToSync *cachedNodeReputationInfo)) {
...
func (cdb *CachingDB) nextTimeForSync(nodeID storx.NodeID, now time.Time) time.Time {
...
func nextTimeForSync(instanceOffset uint64, nodeID storx.NodeID, now time.Time, syncInterval time.Duration) time.Time {
...
func (cdb *CachingDB) Get(ctx context.Context, nodeID storx.NodeID) (info *Info, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cdb *CachingDB) getEntry(ctx context.Context, nodeID storx.NodeID, now time.Time, f func(entry *cachedNodeReputationInfo)) {
	defer mon.Task()(&ctx)(nil)
...
func (cdb *CachingDB) insertNode(nodeID storx.NodeID, now time.Time) *cachedNodeReputationInfo {
