...
func TestAuditSuspendExceedGracePeriod(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		repService := planet.Satellites[0].Reputation.Service
		for _, node := range (storx.NodeIDList{successNodeID, failNodeID, offlineNodeID, unknownNodeID}) {
			err := repService.TestSuspendNodeUnknownAudit(ctx, node, time.Now().Add(-2*time.Hour))
			require.NoError(t, err)
		}

		nodesStatus := make(map[storx.NodeID]overlay.ReputationStatus)
		...
		report := audit.Report{
			Successes:       storx.NodeIDList{successNodeID},
			Fails:           storx.NodeIDList{failNodeID},
			Offlines:        storx.NodeIDList{offlineNodeID},
			Unknown:         storx.NodeIDList{unknownNodeID},
		...
		for _, node := range (storx.NodeIDList{successNodeID, offlineNodeID}) {
			n, err := repService.Get(ctx, node)
			require.NoError(t, err)
			require.Nil(t, n.Disqualified)
		}
		for _, node := range (storx.NodeIDList{failNodeID, unknownNodeID}) {
			n, err := repService.Get(ctx, node)
			require.NoError(t, err)
