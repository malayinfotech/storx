// Copyright (C) 2019 Storx Labs, Incache.
...
func (cache *DownloadSelectionCache) GetNodeIPs(ctx context.Context, nodes []storx.NodeID) (_ map[storx.NodeID]string, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cache *DownloadSelectionCache) GetNodes(ctx context.Context, nodes []storx.NodeID) (_ map[storx.NodeID]*SelectedNode, err error) {
	defer mon.Task()(&ctx)(&err)
...
type DownloadSelectionCacheState struct {
	// byID returns IP based on storx.NodeID
	byID map[storx.NodeID]*SelectedNode // TODO: optimize, avoid pointery structures for performance
}
...
func NewDownloadSelectionCacheState(nodes []*SelectedNode) *DownloadSelectionCacheState {
	byID := map[storx.NodeID]*SelectedNode{}
	for _, n := range nodes {
		byID[n.ID] = n
	}
...
func (state *DownloadSelectionCacheState) IPs(nodes []storx.NodeID) map[storx.NodeID]string {
	xs := make(map[storx.NodeID]string, len(nodes))
...
func (state *DownloadSelectionCacheState) Nodes(nodes []storx.NodeID) map[storx.NodeID]*SelectedNode {
	xs := make(map[storx.NodeID]*SelectedNode, len(nodes))
	for _, nodeID := range nodes {
		if n, exists := state.byID[nodeID]; exists {
