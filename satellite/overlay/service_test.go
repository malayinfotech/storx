...
func testCache(ctx *testcontext.Context, t *testing.T, store overlay.DB, nodeEvents nodeevents.DB) {
	...
	{ // Get
		_, err := service.Get(ctx, storx.NodeID{})
		require.Error(t, err)
...
func TestRandomizedSelection(t *testing.T) {
	...
	satellitedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db satellite.DB) {
		...
		allIDs := make(storx.NodeIDList, totalNodes)
		nodeCounts := make(map[storx.NodeID]int)
...
func TestGetOnlineNodesForGetDelete(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		actualNodes, err := planet.Satellites[0].Overlay.Service.GetOnlineNodesForGetDelete(ctx, []storx.NodeID{})
		require.NoError(t, err)
		require.Equal(t, 0, len(actualNodes))
		actualNodes, err = planet.Satellites[0].Overlay.Service.GetOnlineNodesForGetDelete(ctx, []storx.NodeID{testrand.NodeID()})
		require.NoError(t, err)
		require.Equal(t, 0, len(actualNodes))

		expectedNodes := make(map[storx.NodeID]*overlay.SelectedNode, len(planet.StorageNodes))
		nodeIDs := make([]storx.NodeID, len(planet.StorageNodes)+1)
...
func TestKnownReliable(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		result, err := service.KnownReliable(ctx, []storx.NodeID{
		...
		expectedReliable := []storx.NodeURL{
...
func TestUpdateCheckIn(t *testing.T) {
	satellitedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db satellite.DB) { // setup
		...
		require.NoError(t, db.OverlayCache().UpdateLastOfflineEmail(ctx, []storx.NodeID{updated2Node.Id}, time.Now()))
...
func TestSuspendedSelection(t *testing.T) {
	...
	satellitedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db satellite.DB) {
		cache := db.OverlayCache()
		suspendedIDs := make(map[storx.NodeID]bool)
...
func getNodeInfo(nodeID storx.NodeID) overlay.NodeCheckInInfo {
	return overlay.NodeCheckInInfo{
		NodeID: nodeID,
		IsUp:   true,
		Address: &pb.NodeAddress{
...
func TestUpdateReputationNodeEvents(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		require.NoError(t, service.UpdateReputation(ctx, node.ID(), "test@storx.test", repUpdate, repChanges))
