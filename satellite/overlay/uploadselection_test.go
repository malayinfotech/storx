...
func addNodesToNodesTable(ctx context.Context, t *testing.T, db overlay.DB, count, makeReputable int) (ids []storx.NodeID) {
	for i := 0; i < count; i++ {
		subnet := strconv.Itoa(i) + ".1.2"
		addr := subnet + ".3:8080"
		n := overlay.NodeCheckInInfo{
			NodeID: storx.NodeID{byte(i)},
		...
		if i < makeReputable {
			vettedAt, err := db.TestVetNode(ctx, storx.NodeID{byte(i)})
			require.NoError(t, err)
			require.NoError(t, err)
			require.NotNil(t, vettedAt)
			ids = append(ids, storx.NodeID{byte(i)})
		}
	}
	return ids
}
...
func TestGetNodesConcurrent(t *testing.T) {
	...
	group.Go(func() error {
		...
		for i := range nodes {
			nodes[i].ID = storx.NodeID{byte(i)}
