...
func TestPeerIdentities(t *testing.T) {
	satellitedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db satellite.DB) {
		...
		{ // get multiple with invalid
			list := make(map[storx.NodeID]*identity.PeerIdentity)
			ids := storx.NodeIDList{}
			savedIDs := make(map[storx.NodeID]bool)
			unsavedIDs := make(map[storx.NodeID]bool)

			for i := 0; i < 10; i++ {
				ident := testidentity.MustPregeneratedSignedIdentity(i, storx.LatestIDVersion())
