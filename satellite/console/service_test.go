	...
	testplanet.Run(t, testplanet.Config{
		...
		func(t *testing.T, ctx *testcontext.Context, planet *testplanet.Planet) {
			...
			t.Run("GetAllBucketNames", func(t *testing.T) {
				bucket1 := storx.Bucket{
					ID:        testrand.UUID(),
					Name:      "testBucket1",
					ProjectID: up2Pro1.ID,
				}

				bucket2 := storx.Bucket{
					ID:        testrand.UUID(),
					Name:      "testBucket2",
					ProjectID: up2Pro1.ID,
				}
	...
	testplanet.Run(t, testplanet.Config{
		...
		for i := 0; i < 5; i++ {
			tx := stripecoinpayments.Transaction{
				...
				Amount:    currency.AmountFromBaseUnits(1000000000, currency.StorxToken),
				Received:  currency.AmountFromBaseUnits(1000000000, currency.StorxToken),
		...
		err = sat.DB.StorxscanPayments().InsertBatch(ctx, cachedPayments)
		require.NoError(t, err)
