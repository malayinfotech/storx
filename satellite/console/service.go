...
func (payment Payments) BillingHistory(ctx context.Context) (billingHistory []*BillingHistoryItem, err error) {
	...
	txsInfos, err := payment.service.accounts.StorxTokens().ListTransactionInfos(ctx, user.ID)
	if err != nil {
		return nil, Error.Wrap(err)
	}
	...
	bonuses, err := payment.service.accounts.StorxTokens().ListDepositBonuses(ctx, user.ID)
	if err != nil {
		return nil, Error.Wrap(err)
	}
	...
	for _, bonus := range bonuses {
		billingHistory = append(billingHistory,
			&BillingHistoryItem{
				Description: fmt.Sprintf("%d%% Bonus for STORX Token Deposit", bonus.Percentage),
...
func (s *Service) GetAllBucketNames(ctx context.Context, projectID uuid.UUID) (_ []string, err error) {
	...
	listOptions := storx.BucketListOptions{
		Direction: storx.Forward,
	}
...
// storx/storx/satellite/accounting.Service, wrapped Error.
...
func (payment Payments) WalletPayments(ctx context.Context) (_ WalletPayments, err error) {
	...
	txInfos, err := payment.service.accounts.StorxTokens().ListTransactionInfos(ctx, user.ID)
	if err != nil {
		return WalletPayments{}, Error.Wrap(err)
	}
