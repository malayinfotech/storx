...
func TestAuth_Register_CORS(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		// 1. OPTIONS request
		//     1.1 CORS headers should not be set with origin other than storx or www.storx
		...
		//     1.2 CORS headers should be set with a domain of storx
		req.Header.Set("Origin", "https://storx")
		resp, err = http.DefaultClient.Do(req)
		require.NoError(t, err)
		require.Equal(t, http.StatusOK, resp.StatusCode)
		require.Equal(t, "https://storx", resp.Header.Get("Access-Control-Allow-Origin"))
		...
		//     1.3 CORS headers should be set with a domain of www.storx
		req.Header.Set("Origin", "https://www.storx")
		resp, err = http.DefaultClient.Do(req)
		require.NoError(t, err)
		require.Equal(t, http.StatusOK, resp.StatusCode)
		require.Equal(t, "https://www.storx", resp.Header.Get("Access-Control-Allow-Origin"))
		require.Equal(t, "POST, OPTIONS", resp.Header.Get("Access-Control-Allow-Methods"))
		allowedHeaders = strings.Split(resp.Header.Get("Access-Control-Allow-Headers"), ", ")
		require.ElementsMatch(t, allowedHeaders, []string{
			"Content-Type",
		...
		// 2. POST request with origin www.storx
...
func TestDeleteAccount(t *testing.T) {
	...
	config := &quick.Config{
		Values: func(values []reflect.Value, rnd *rand.Rand) {
			...
			reqURL, err := url.Parse("//storx/" + path + withQuery + query)
