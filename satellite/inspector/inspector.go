...
func (endpoint *Endpoint) segmentHealth(ctx context.Context, segment metabase.Segment) (_ *internalpb.SegmentHealthResponse, err error) {
	...
	offlineMap := make(map[storx.NodeID]bool)
	for _, id := range offlineNodes {
		offlineMap[id] = true
	}
	unreliableOfflineMap := make(map[storx.NodeID]bool)
	...
	var healthyNodes storx.NodeIDList
	var unhealthyNodes storx.NodeIDList
