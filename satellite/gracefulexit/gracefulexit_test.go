...
func TestGracefulExit_CopiedObjects(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		testGETransferQueue := func(node storx.NodeID) {
...
func generateExitedNodes(t *testing.T, ctx *testcontext.Context, db satellite.DB, currentTime time.Time, numExitedNodes int) (exitedNodeIDs storx.NodeIDList) {
	const (
		addr    = "127.0.1.0:8080"
		lastNet = "127.0.0"
	)
	var (
		...
		nodeIDsMap = make(map[storx.NodeID]struct{})
	)
...
func TestGracefulExit_DeleteAllFinishedTransferQueueItems_batch(t *testing.T) {
	satellitedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db satellite.DB) {
		...
		var (
			currentTime   = time.Now().UTC()
			exitedNodeIDs = make([]storx.NodeID, 0, numNonExitedNodes)
			nodeIDsMap    = make(map[storx.NodeID]struct{})
		)
...
func generateTransferQueueItems(t *testing.T, itemsPerNode int, nodes []*testplanet.StorageNode) ([]gracefulexit.TransferQueueItem, map[storx.NodeID]int64) {
	nodeIDs := make([]storx.NodeID, len(nodes))
	for i, n := range nodes {
		nodeIDs[i] = n.ID()
	}
	...
	nodesItems := make(map[storx.NodeID]int64, len(nodes))
...
func generateNTransferQueueItemsPerNode(t *testing.T, n int, nodeIDs ...storx.NodeID) []gracefulexit.TransferQueueItem {
...
func generateNodeIDFromPostiveInt(t *testing.T, val int) storx.NodeID {
	t.Helper()
