...
func (pm *connectionsTracker) delete(nodeID storx.NodeID) {
	pm.mu.Lock()
	defer pm.mu.Unlock()
...
func (endpoint *Endpoint) processIncomplete(ctx context.Context, stream pb.DRPCSatelliteGracefulExit_ProcessStream, pending *PendingMap, incomplete *TransferQueueItem) error {
	...
	excludedIDs := make([]storx.NodeID, len(pieces))
...
func (endpoint *Endpoint) handleSucceeded(ctx context.Context, stream pb.DRPCSatelliteGracefulExit_ProcessStream, pending *PendingMap, exitingNodeID storx.NodeID, message *pb.StorageNodeMessage_Succeeded) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (endpoint *Endpoint) handleFailed(ctx context.Context, pending *PendingMap, nodeID storx.NodeID, message *pb.StorageNodeMessage_Failed) (err error) {
	defer mon.Task()(&ctx)(&err)
	...
	if pb.TransferFailed_Error(errorCode) == pb.TransferFailed_NOT_FOUND {
		...
		segment, err := endpoint.getValidSegment(ctx, transfer.StreamID, transfer.Position, storx.PieceID{})
		if err != nil {
			return Error.Wrap(err)
		}
...
func (endpoint *Endpoint) handleDisqualifiedNode(ctx context.Context, nodeID storx.NodeID) (isDisqualified bool, err error) {
...
func (endpoint *Endpoint) getFinishedMessage(ctx context.Context, nodeID storx.NodeID, finishedAt time.Time, success bool, reason pb.ExitFailed_Reason) (message *pb.SatelliteMessage, err error) {
	if success {
...
func (endpoint *Endpoint) updateSegment(ctx context.Context, exitingNodeID storx.NodeID, receivingNodeID storx.NodeID, streamID uuid.UUID, position metabase.SegmentPosition, pieceNumber uint16, originalRootPieceID storx.PieceID) (err error) {
	defer mon.Task()(&ctx)(&err)
	...
	pieceMap := make(map[storx.NodeID]metabase.Piece)
...
func (endpoint *Endpoint) checkExitStatus(ctx context.Context, nodeID storx.NodeID) (*pb.SatelliteMessage, error) {
...
func (endpoint *Endpoint) generateExitStatusRequest(ctx context.Context, nodeID storx.NodeID) (*overlay.ExitStatusRequest, pb.ExitFailed_Reason, error) {
...
func (endpoint *Endpoint) getValidSegment(ctx context.Context, streamID uuid.UUID, position metabase.SegmentPosition, originalRootPieceID storx.PieceID) (metabase.Segment, error) {
...
func (endpoint *Endpoint) UpdatePiecesCheckDuplicates(ctx context.Context, segment metabase.Segment, toAdd, toRemove metabase.Pieces, checkDuplicates bool) (err error) {
	...
	if checkDuplicates {
		...
		nodePieceMap := make(map[storx.NodeID]struct{})
		for _, piece := range segment.Pieces {
	...
	if err != nil {
		if metabase.ErrSegmentNotFound.Has(err) {
			err = storx.ErrObjectNotFound.Wrap(err)
		}
		return Error.Wrap(err)
	}
