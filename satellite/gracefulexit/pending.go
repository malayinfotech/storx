...
type PendingTransfer struct {
	...
	OriginalRootPieceID storx.PieceID
	PieceNum            uint16
}
...
type PendingMap struct {
	mu              sync.RWMutex
	data            map[storx.PieceID]*PendingTransfer
	doneSending     bool
...
func NewPendingMap() *PendingMap {
	newData := make(map[storx.PieceID]*PendingTransfer)
	return &PendingMap{
		data: newData,
	}
}
...
func (pm *PendingMap) Put(pieceID storx.PieceID, pendingTransfer *PendingTransfer) error {
	pm.mu.Lock()
	defer pm.mu.Unlock()
...
func (pm *PendingMap) Get(pieceID storx.PieceID) (*PendingTransfer, bool) {
	pm.mu.RLock()
	defer pm.mu.RUnlock()
...
func (pm *PendingMap) Delete(pieceID storx.PieceID) error {
	pm.mu.Lock()
	defer pm.mu.Unlock()
