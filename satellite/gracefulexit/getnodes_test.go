...
func TestGetExitingNodes(t *testing.T) {
	satellitedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db satellite.DB) {
		...
		exiting := make(map[storx.NodeID]bool)
		exitingCount := 0
		exitingLoopIncomplete := make(map[storx.NodeID]bool)
		...
		nodes, err = cache.GetExitingNodes(ctx)
		require.NoError(t, err)
		exitingNodesLoopIncomplete := make(storx.NodeIDList, 0, len(nodes))
		for _, node := range nodes {
...
func TestGetGracefulExitNodesByTimeframe(t *testing.T) {
	satellitedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db satellite.DB) {
		cache := db.OverlayCache()
		exitingToday := make(map[storx.NodeID]bool)
		exitingLastWeek := make(map[storx.NodeID]bool)
		exitedToday := make(map[storx.NodeID]bool)
		exitedLastWeek := make(map[storx.NodeID]bool)
