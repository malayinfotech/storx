...
// reverifyQueue implements storx/storx/satellite/audit.ReverifyQueue.
...
func (rq *reverifyQueue) GetByNodeID(ctx context.Context, nodeID storx.NodeID) (pendingJob *audit.ReverificationJob, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (rq *reverifyQueue) GetAllContainedNodes(ctx context.Context) (nodes []storx.NodeID, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	for result.Next() {
		...
		nodeID, err := storx.NodeIDFromBytes(nodeIDBytes)
		if err != nil {
			return nil, audit.ContainError.Wrap(err)
		}
...
func convertDBJob(ctx context.Context, info *dbx.ReverificationAudits) (pendingJob *audit.ReverificationJob, err error) {
	...
	pendingJob.Locator.NodeID, err = storx.NodeIDFromBytes(info.NodeId)
	if err != nil {
		return nil, audit.ContainError.Wrap(err)
	}
