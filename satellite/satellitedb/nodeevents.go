...
func (ne *nodeEvents) Insert(ctx context.Context, email string, nodeID storx.NodeID, eventType nodeevents.Type) (nodeEvent nodeevents.NodeEvent, err error) {
...
func fromDBX(dbxNE *dbx.NodeEvent) (event nodeevents.NodeEvent, err error) {
	...
	nodeID, err := storx.NodeIDFromBytes(dbxNE.NodeId)
	if err != nil {
		return event, err
	}
