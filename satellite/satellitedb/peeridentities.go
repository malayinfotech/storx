...
func (idents *peerIdentities) Set(ctx context.Context, nodeID storx.NodeID, ident *identity.PeerIdentity) (err error) {
...
func (idents *peerIdentities) Get(ctx context.Context, nodeID storx.NodeID) (_ *identity.PeerIdentity, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (idents *peerIdentities) BatchGet(ctx context.Context, nodeIDs storx.NodeIDList) (peerIdents []*identity.PeerIdentity, err error) {
	defer mon.Task()(&ctx)(&err)
	if len(nodeIDs) == 0 {
		return nil, nil
	}
