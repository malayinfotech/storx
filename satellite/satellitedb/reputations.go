...
// responsible for updating the records in the overlay to match.
func (reputations *reputations) ApplyUpdates(ctx context.Context, nodeID storx.NodeID, updates reputation.Mutations, reputationConfig reputation.Config, now time.Time) (_ *reputation.Info, err error) {
...
func (reputations *reputations) Get(ctx context.Context, nodeID storx.NodeID) (*reputation.Info, error) {
...
func (reputations *reputations) DisqualifyNode(ctx context.Context, nodeID storx.NodeID, disqualifiedAt time.Time, disqualificationReason overlay.DisqualificationReason) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (reputations *reputations) SuspendNodeUnknownAudit(ctx context.Context, nodeID storx.NodeID, suspendedAt time.Time) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (reputations *reputations) UnsuspendNodeUnknownAudit(ctx context.Context, nodeID storx.NodeID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (z zapNodeIDBytes) String() string {
	nodeID, err := storx.NodeIDFromBytes([]byte(z))
	if err != nil {
