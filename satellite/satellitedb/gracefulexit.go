...
func (db *gracefulexitDB) IncrementProgress(ctx context.Context, nodeID storx.NodeID, bytes int64, successfulTransfers int64, failedTransfers int64) (err error) {
...
func (db *gracefulexitDB) GetProgress(ctx context.Context, nodeID storx.NodeID) (_ *gracefulexit.Progress, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	nID, err := storx.NodeIDFromBytes(dbxProgress.NodeId)
	if err != nil {
		return nil, Error.Wrap(err)
	}
...
func (db *gracefulexitDB) DeleteTransferQueueItem(ctx context.Context, nodeID storx.NodeID, streamID uuid.UUID, position metabase.SegmentPosition, pieceNum int32) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (db *gracefulexitDB) DeleteTransferQueueItems(ctx context.Context, nodeID storx.NodeID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (db *gracefulexitDB) DeleteFinishedTransferQueueItems(ctx context.Context, nodeID storx.NodeID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (db *gracefulexitDB) GetFinishedExitNodes(ctx context.Context, before time.Time, asOfSystemInterval time.Duration) (finishedNodes []storx.NodeID, err error) {
	defer mon.Task()(&ctx)(&err)
	stmt := `
		SELECT id
		FROM nodes
...
// affect correctness.
func (db *gracefulexitDB) DeleteBatchExitProgress(ctx context.Context, nodeIDs []storx.NodeID) (deleted int64, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (db *gracefulexitDB) GetTransferQueueItem(ctx context.Context, nodeID storx.NodeID, streamID uuid.UUID, position metabase.SegmentPosition, pieceNum int32) (_ *gracefulexit.TransferQueueItem, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (db *gracefulexitDB) GetIncomplete(ctx context.Context, nodeID storx.NodeID, limit int, offset int64) (_ []*gracefulexit.TransferQueueItem, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (db *gracefulexitDB) GetIncompleteNotFailed(ctx context.Context, nodeID storx.NodeID, limit int, offset int64) (_ []*gracefulexit.TransferQueueItem, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (db *gracefulexitDB) GetIncompleteFailed(ctx context.Context, nodeID storx.NodeID, maxFailures int, limit int, offset int64) (_ []*gracefulexit.TransferQueueItem, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (db *gracefulexitDB) IncrementOrderLimitSendCount(ctx context.Context, nodeID storx.NodeID, streamID uuid.UUID, position metabase.SegmentPosition, pieceNum int32) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (db *gracefulexitDB) CountFinishedTransferQueueItemsByNode(ctx context.Context, before time.Time, asOfSystemInterval time.Duration) (_ map[storx.NodeID]int64, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	nodesItemsCount := make(map[storx.NodeID]int64)
	for rows.Next() {
		var (
			nodeID storx.NodeID
			n      int64
...
func scanRows(rows tagsql.Rows) (transferQueueItemRows []*gracefulexit.TransferQueueItem, err error) {
	for rows.Next() {
		...
		if pieceIDBytes != nil {
			transferQueueItem.RootPieceID, err = storx.PieceIDFromBytes(pieceIDBytes)
			if err != nil {
				return nil, Error.Wrap(err)
			}
		}
...
func dbxSegmentTransferToTransferQueueItem(dbxSegmentTransfer *dbx.GracefulExitSegmentTransfer) (item *gracefulexit.TransferQueueItem, err error) {
	nID, err := storx.NodeIDFromBytes(dbxSegmentTransfer.NodeId)
	if err != nil {
		return nil, Error.Wrap(err)
	}
	...
	if dbxSegmentTransfer.RootPieceId != nil {
		item.RootPieceID, err = storx.PieceIDFromBytes(dbxSegmentTransfer.RootPieceId)
		if err != nil {
			return nil, err
		}
	}
