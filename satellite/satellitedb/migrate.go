...
func (db *satelliteDB) ProductionMigration() *migrate.Migration {
	return &migrate.Migration{
		...
		Steps: []*migrate.Step{
			...
			{
				DB:          &db.migrationDB,
				Description: "add storxscan_wallets",
				Version:     198,
				Action: migrate.SQL{
					`CREATE TABLE storxscan_wallets (
						user_id bytea NOT NULL,
			...
			{
				DB:          &db.migrationDB,
				Description: "add storxscan_payments table and index on block number and log index",
				Version:     200,
				Action: migrate.SQL{
					`CREATE TABLE storxscan_payments (
					...
					`CREATE INDEX storxscan_payments_block_number_log_index_index ON storxscan_payments ( block_number, log_index );`,
				},
			},
			{
				DB:          &db.migrationDB,
			...
			},
			{
				...
				SeparateTx:  true,
				Action: migrate.SQL{
					`CREATE INDEX IF NOT EXISTS storxscan_wallets_wallet_address_index ON storxscan_wallets ( wallet_address );`,
				},
			},
			{
				DB:          &db.migrationDB,
