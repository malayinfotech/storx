...
func (cache *overlaycache) GetNodesNetwork(ctx context.Context, nodeIDs []storx.NodeID) (nodeNets []string, err error) {
	for {
...
func (cache *overlaycache) Get(ctx context.Context, id storx.NodeID) (dossier *overlay.NodeDossier, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cache *overlaycache) GetOnlineNodesForGetDelete(ctx context.Context, nodeIDs []storx.NodeID, onlineWindow time.Duration, asOf overlay.AsOfSystemTimeConfig) (nodes map[storx.NodeID]*overlay.SelectedNode, err error) {
	for {
...
func (cache *overlaycache) getOnlineNodesForGetDelete(ctx context.Context, nodeIDs []storx.NodeID, onlineWindow time.Duration, asOf overlay.AsOfSystemTimeConfig) (_ map[storx.NodeID]*overlay.SelectedNode, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	nodes := make(map[storx.NodeID]*overlay.SelectedNode)
	for rows.Next() {
		var node overlay.SelectedNode
		node.Address = &pb.NodeAddress{}
...
func (cache *overlaycache) GetOnlineNodesForAuditRepair(ctx context.Context, nodeIDs []storx.NodeID, onlineWindow time.Duration) (nodes map[storx.NodeID]*overlay.NodeReputation, err error) {
	for {
...
func (cache *overlaycache) getOnlineNodesForAuditRepair(ctx context.Context, nodeIDs []storx.NodeID, onlineWindow time.Duration) (_ map[storx.NodeID]*overlay.NodeReputation, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	nodes := make(map[storx.NodeID]*overlay.NodeReputation)
	for rows.Next() {
...
func (cache *overlaycache) KnownOffline(ctx context.Context, criteria *overlay.NodeCriteria, nodeIDs storx.NodeIDList) (offlineNodes storx.NodeIDList, err error) {
	for {
...
func (cache *overlaycache) KnownUnreliableOrOffline(ctx context.Context, criteria *overlay.NodeCriteria, nodeIDs storx.NodeIDList) (badNodes storx.NodeIDList, err error) {
	for {
...
func (cache *overlaycache) GetOfflineNodesForEmail(ctx context.Context, offlineWindow, cutoff, cooldown time.Duration, limit int) (nodes map[storx.NodeID]string, err error) {
	defer mon.Task()(&ctx)(&err)

	now := time.Now()
	nodes = make(map[storx.NodeID]string)
	...
	for rows.Next() {
		...
		nodeID, err = storx.NodeIDFromBytes(idBytes)
		if err != nil {
			return
		}
		nodes[nodeID] = email
...
func (cache *overlaycache) UpdateLastOfflineEmail(ctx context.Context, nodeIDs storx.NodeIDList, timestamp time.Time) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cache *overlaycache) KnownReliableInExcludedCountries(ctx context.Context, criteria *overlay.NodeCriteria, nodeIDs storx.NodeIDList) (reliableInExcluded storx.NodeIDList, err error) {
	for {
...
func (cache *overlaycache) knownUnreliableOrOffline(ctx context.Context, criteria *overlay.NodeCriteria, nodeIDs storx.NodeIDList) (badNodes storx.NodeIDList, err error) {
	...
	goodNodes := make(map[storx.NodeID]struct{}, len(nodeIDs))
	for rows.Next() {
		var id storx.NodeID
		err = rows.Scan(&id)
		if err != nil {
...
func (cache *overlaycache) KnownReliable(ctx context.Context, onlineWindow time.Duration, nodeIDs storx.NodeIDList) (nodes []*pb.Node, err error) {
	for {
...
func (cache *overlaycache) Reliable(ctx context.Context, criteria *overlay.NodeCriteria) (nodes storx.NodeIDList, err error) {
	for {
...
func (cache *overlaycache) UpdateReputation(ctx context.Context, id storx.NodeID, request overlay.ReputationUpdate) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cache *overlaycache) UpdateNodeInfo(ctx context.Context, nodeID storx.NodeID, nodeInfo *overlay.InfoResponse) (stats *overlay.NodeDossier, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cache *overlaycache) DisqualifyNode(ctx context.Context, nodeID storx.NodeID, disqualifiedAt time.Time, reason overlay.DisqualificationReason) (email string, err error) {
	defer mon.Task()(&ctx)(&err)
	updateFields := dbx.Node_Update_Fields{}
...
func (cache *overlaycache) TestSuspendNodeUnknownAudit(ctx context.Context, nodeID storx.NodeID, suspendedAt time.Time) (err error) {
	defer mon.Task()(&ctx)(&err)
	updateFields := dbx.Node_Update_Fields{}
...
func (cache *overlaycache) TestUnsuspendNodeUnknownAudit(ctx context.Context, nodeID storx.NodeID) (err error) {
	defer mon.Task()(&ctx)(&err)
	updateFields := dbx.Node_Update_Fields{}
...
func (cache *overlaycache) AllPieceCounts(ctx context.Context) (_ map[storx.NodeID]int64, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	pieceCounts := make(map[storx.NodeID]int64)
	nodeIDErrs := errs.Group{}
	for _, row := range rows {
		nodeID, err := storx.NodeIDFromBytes(row.Id)
		if err != nil {
			nodeIDErrs.Add(err)
			continue
		}
...
func (cache *overlaycache) UpdatePieceCounts(ctx context.Context, pieceCounts map[storx.NodeID]int64) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cache *overlaycache) GetExitStatus(ctx context.Context, nodeID storx.NodeID) (exitStatus *overlay.ExitStatus, err error) {
	for {
...
func (cache *overlaycache) getExitStatus(ctx context.Context, nodeID storx.NodeID) (_ *overlay.ExitStatus, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cache *overlaycache) GetGracefulExitCompletedByTimeFrame(ctx context.Context, begin, end time.Time) (exitedNodes storx.NodeIDList, err error) {
	for {
...
func (cache *overlaycache) GetGracefulExitIncompleteByTimeFrame(ctx context.Context, begin, end time.Time) (exitingNodes storx.NodeIDList, err error) {
	for {
...
func convertDBNode(ctx context.Context, info *dbx.Node) (_ *overlay.NodeDossier, err error) {
	...
	id, err := storx.NodeIDFromBytes(info.Id)
	if err != nil {
		return nil, err
	}
...
func (cache *overlaycache) DQNodesLastSeenBefore(ctx context.Context, cutoff time.Time, limit int) (nodeEmails map[storx.NodeID]string, count int, err error) {
	defer mon.Task()(&ctx)(&err)

	var nodeIDs []storx.NodeID
	nodeEmails = make(map[storx.NodeID]string)
	for {
...
func (cache *overlaycache) getNodesForDQLastSeenBefore(ctx context.Context, cutoff time.Time, limit int) (nodes []storx.NodeID, err error) {
	defer mon.Task()(&ctx)(&err)
...
// alone.
func (cache *overlaycache) SetNodeContained(ctx context.Context, nodeID storx.NodeID, contained bool) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cache *overlaycache) SetAllContainedNodes(ctx context.Context, containedNodes []storx.NodeID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cache *overlaycache) TestVetNode(ctx context.Context, nodeID storx.NodeID) (vettedTime *time.Time, err error) {
	updateFields := dbx.Node_Update_Fields{
...
func (cache *overlaycache) TestUnvetNode(ctx context.Context, nodeID storx.NodeID) (err error) {
...
func (cache *overlaycache) TestSuspendNodeOffline(ctx context.Context, nodeID storx.NodeID, suspendedAt time.Time) (err error) {
	defer mon.Task()(&ctx)(&err)
	updateFields := dbx.Node_Update_Fields{}
...
func (cache *overlaycache) TestNodeCountryCode(ctx context.Context, nodeID storx.NodeID, countryCode string) (err error) {
	defer mon.Task()(&ctx)(&err)
	updateFields := dbx.Node_Update_Fields{}
...
func (cache *overlaycache) IterateAllContactedNodes(ctx context.Context, cb func(context.Context, *overlay.SelectedNode) error) (err error) {
	...
	var rows tagsql.Rows
	// 2018-04-06 is the date of the first storx v3 commit.
	rows, err = cache.db.Query(ctx, cache.db.Rebind(`
		SELECT last_net, id, address, last_ip_port, noise_proto, noise_public_key, debounce_limit
		FROM nodes
