...
type DB interface {
	...
	CreateBucket(ctx context.Context, bucket storx.Bucket) (_ storx.Bucket, err error)
	// GetBucket returns an existing bucket
	GetBucket(ctx context.Context, bucketName []byte, projectID uuid.UUID) (bucket storx.Bucket, err error)
	// GetBucketPlacement returns with the placement constraint identifier.
	GetBucketPlacement(ctx context.Context, bucketName []byte, projectID uuid.UUID) (placement storx.PlacementConstraint, err error)
	...
	UpdateBucket(ctx context.Context, bucket storx.Bucket) (_ storx.Bucket, err error)
	...
	ListBuckets(ctx context.Context, projectID uuid.UUID, listOpts storx.BucketListOptions, allowedBuckets macaroon.AllowedBuckets) (bucketList storx.BucketList, err error)
