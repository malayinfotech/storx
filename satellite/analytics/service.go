...
const (
	...
	eventModalAddTokens               = "Storx Token Added In Modal"
	...
	eventStorxTokenAdded              = "Storx Token Added"
	...
	eventStorxTokenAddedFromBilling   = "Storx Token Added From Billing"
...
func NewService(log *zap.Logger, config Config, satelliteName string) *Service {
	...
	for _, name := range []string{eventGatewayCredentialsCreated, eventPassphraseCreated, eventExternalLinkClicked,
		...
		eventStorxTokenAddedFromBilling, eventAddFundsClicked, eventProjectMembersInviteSent, eventError, eventProjectNameUpdated, eventProjectDescriptionUpdated,
...
// TrackStorxTokenAdded sends an "Storx Token Added" event to Segment.
func (service *Service) TrackStorxTokenAdded(userID uuid.UUID, email string) {
	if !service.config.Enabled {
		return
	}
	...
	service.enqueueMessage(segment.Track{
		UserId:     userID.String(),
		Event:      service.satelliteName + " " + eventStorxTokenAdded,
		Properties: props,
	})
