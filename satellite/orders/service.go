...
type Overlay interface {
	CachedGetOnlineNodesForGet(context.Context, []storx.NodeID) (map[storx.NodeID]*overlay.SelectedNode, error)
	GetOnlineNodesForAuditRepair(context.Context, []storx.NodeID) (map[storx.NodeID]*overlay.NodeReputation, error)
	Get(ctx context.Context, nodeID storx.NodeID) (*overlay.NodeDossier, error)
...
func (service *Service) CreateGetOrderLimits(ctx context.Context, bucket metabase.BucketLocation, segment metabase.Segment, desiredNodes int32, overrideLimit int64) (_ []*pb.AddressedOrderLimit, privateKey storx.PiecePrivateKey, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	nodeIDs := make([]storx.NodeID, len(segment.Pieces))
	...
	if err != nil {
		...
		return nil, storx.PiecePrivateKey{}, Error.Wrap(err)
	}
	...
	if len(signer.AddressedLimits) < int(segment.Redundancy.RequiredShares) {
		...
		return nil, storx.PiecePrivateKey{}, ErrDownloadFailedNotEnoughPieces.New("not enough orderlimits: got %d, required %d", len(signer.AddressedLimits), segment.Redundancy.RequiredShares)
	}
	...
	if err != nil {
		return nil, storx.PiecePrivateKey{}, err
	}
...
func getLimitByStorageNodeID(limits []*pb.AddressedOrderLimit, storageNodeID storx.NodeID) *pb.AddressedOrderLimit {
...
func (service *Service) CreatePutOrderLimits(ctx context.Context, bucket metabase.BucketLocation, nodes []*overlay.SelectedNode, pieceExpiration time.Time, maxPieceSize int64) (_ storx.PieceID, _ []*pb.AddressedOrderLimit, privateKey storx.PiecePrivateKey, err error) {
	defer mon.Task()(&ctx)(&err)

	signer, err := NewSignerPut(service, pieceExpiration, time.Now(), maxPieceSize, bucket)
	if err != nil {
		return storx.PieceID{}, nil, storx.PiecePrivateKey{}, Error.Wrap(err)
	}
...
func (service *Service) ReplacePutOrderLimits(ctx context.Context, rootPieceID storx.PieceID, addressedLimits []*pb.AddressedOrderLimit, nodes []*overlay.SelectedNode, pieceNumbers []int32) (_ []*pb.AddressedOrderLimit, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (service *Service) CreateAuditOrderLimits(ctx context.Context, segment metabase.Segment, skip map[storx.NodeID]bool) (_ []*pb.AddressedOrderLimit, _ storx.PiecePrivateKey, cachedNodesInfo map[storx.NodeID]overlay.NodeReputation, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	if err != nil {
		service.log.Debug("error getting nodes from overlay", zap.Error(err))
		return nil, storx.PiecePrivateKey{}, nil, Error.Wrap(err)
	}
	...
	cachedNodesInfo = make(map[storx.NodeID]overlay.NodeReputation)
	...
	if limitsCount < segment.Redundancy.RequiredShares {
		...
		return nil, storx.PiecePrivateKey{}, nil, errs.Combine(err, nodeErrors.Err())
	}
...
func (service *Service) CreateAuditOrderLimit(ctx context.Context, nodeID storx.NodeID, pieceNum uint16, rootPieceID storx.PieceID, shareSize int32) (limit *pb.AddressedOrderLimit, _ storx.PiecePrivateKey, nodeInfo *overlay.NodeReputation, err error) {
	...
	if err != nil {
		return nil, storx.PiecePrivateKey{}, nodeInfo, Error.Wrap(err)
	}
...
func (service *Service) CreateAuditPieceOrderLimit(ctx context.Context, nodeID storx.NodeID, pieceNum uint16, rootPieceID storx.PieceID, pieceSize int32) (limit *pb.AddressedOrderLimit, _ storx.PiecePrivateKey, nodeInfo *overlay.NodeReputation, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (service *Service) createAuditOrderLimitWithSigner(ctx context.Context, nodeID storx.NodeID, pieceNum uint16, signer *Signer) (limit *pb.AddressedOrderLimit, _ storx.PiecePrivateKey, nodeInfo *overlay.NodeReputation, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	if node.Disqualified != nil {
		return nil, storx.PiecePrivateKey{}, nodeInfo, overlay.ErrNodeDisqualified.New("%v", nodeID)
	}
	if node.ExitStatus.ExitFinishedAt != nil {
		return nil, storx.PiecePrivateKey{}, nodeInfo, overlay.ErrNodeFinishedGE.New("%v", nodeID)
	}
	if !service.overlay.IsOnline(node) {
		return nil, storx.PiecePrivateKey{}, nodeInfo, overlay.ErrNodeOffline.New("%v", nodeID)
	}
...
func (service *Service) CreateGetRepairOrderLimits(ctx context.Context, bucket metabase.BucketLocation, segment metabase.Segment, healthy metabase.Pieces) (_ []*pb.AddressedOrderLimit, _ storx.PiecePrivateKey, cachedNodesInfo map[storx.NodeID]overlay.NodeReputation, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	cachedNodesInfo = make(map[storx.NodeID]overlay.NodeReputation, len(healthy))
	var nodeErrors errs.Group
	var limitsCount int
...
func (service *Service) CreatePutRepairOrderLimits(ctx context.Context, bucket metabase.BucketLocation, segment metabase.Segment, getOrderLimits []*pb.AddressedOrderLimit, newNodes []*overlay.SelectedNode, optimalThresholdMultiplier float64, numPiecesInExcludedCountries int) (_ []*pb.AddressedOrderLimit, _ storx.PiecePrivateKey, err error) {
	...
	for _, node := range newNodes {
		...
		if int(pieceNum) >= totalPieces { // should not happen
			return nil, storx.PiecePrivateKey{}, Error.New("piece num greater than total pieces: %d >= %d", pieceNum, totalPieces)
		}
...
func (service *Service) CreateGracefulExitPutOrderLimit(ctx context.Context, bucket metabase.BucketLocation, nodeID storx.NodeID, pieceNum int32, rootPieceID storx.PieceID, shareSize int32) (limit *pb.AddressedOrderLimit, _ storx.PiecePrivateKey, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	if node.Disqualified != nil {
		return nil, storx.PiecePrivateKey{}, overlay.ErrNodeDisqualified.New("%v", nodeID)
	}
	if !service.overlay.IsOnline(node) {
		return nil, storx.PiecePrivateKey{}, overlay.ErrNodeOffline.New("%v", nodeID)
	}
