...
type DB interface {
	...
	UpdateStoragenodeBandwidthSettle(ctx context.Context, storageNode storx.NodeID, action pb.PieceAction, amount int64, intervalStart time.Time) error
	// UpdateStoragenodeBandwidthSettleWithWindow updates 'settled' bandwidth for given storage node
	UpdateStoragenodeBandwidthSettleWithWindow(ctx context.Context, storageNodeID storx.NodeID, actionAmounts map[int32]int64, window time.Time) (status pb.SettlementWithWindowResponse_Status, alreadyProcessed bool, err error)
	...
	GetStorageNodeBandwidth(ctx context.Context, nodeID storx.NodeID, from, to time.Time) (int64, error)
}
...
func (noopDB) UpdateStoragenodeBandwidthSettle(ctx context.Context, storageNode storx.NodeID, action pb.PieceAction, amount int64, intervalStart time.Time) error {
	return nil
}

func (noopDB) UpdateStoragenodeBandwidthSettleWithWindow(ctx context.Context, storageNodeID storx.NodeID, actionAmounts map[int32]int64, window time.Time) (status pb.SettlementWithWindowResponse_Status, alreadyProcessed bool, err error) {
...
func (noopDB) GetStorageNodeBandwidth(ctx context.Context, nodeID storx.NodeID, from, to time.Time) (int64, error) {
	return 0, nil
}
...
func (endpoint *Endpoint) SettlementWithWindowFinal(stream pb.DRPCOrders_SettlementWithWindowStream) (err error) {
	...
	seenSerials := map[storx.SerialNumber]struct{}{}
...
func (endpoint *Endpoint) isValid(ctx context.Context, log *zap.Logger, order *pb.Order,
	orderLimit *pb.OrderLimit, peerID storx.NodeID, window int64) bool {
