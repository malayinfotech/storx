	...
	testplanet.Run(t, testplanet.Config{
		...
		for _, tt := range testCases {
			func() {
				...
				conn, err := storagenode.Dialer.DialNodeURL(ctx, storx.NodeURL{ID: satellite.ID(), Address: satellite.Addr()})
				require.NoError(t, err)
...
func TestSettlementWithWindowEndpointErrors(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		piecePublicKey1, piecePrivateKey1, err := storx.NewPieceKey()
		require.NoError(t, err)

		_, piecePrivateKey2, err := storx.NewPieceKey()
		require.NoError(t, err)
