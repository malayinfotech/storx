...
type EncryptionKeys struct {
	...
	List    []EncryptionKey
	KeyByID map[EncryptionKeyID]storx.Key
}
...
var _ = ([16]byte)(storx.SerialNumber{})

func serialToNonce(serial storx.SerialNumber) (x [24]byte) {
	copy(x[:], serial[:])
	return x
}

// Encrypt encrypts data and nonce using the key.
func (key *EncryptionKey) Encrypt(plaintext []byte, nonce storx.SerialNumber) []byte {
...
func (key *EncryptionKey) Decrypt(ciphertext []byte, nonce storx.SerialNumber) ([]byte, error) {
...
func (key *EncryptionKey) EncryptMetadata(serial storx.SerialNumber, metadata *internalpb.OrderLimitMetadata) ([]byte, error) {
...
func (key *EncryptionKey) DecryptMetadata(serial storx.SerialNumber, encrypted []byte) (*internalpb.OrderLimitMetadata, error) {
...
func (keys *EncryptionKeys) Add(ekey EncryptionKey) error {
	if keys.KeyByID == nil {
		keys.KeyByID = map[EncryptionKeyID]storx.Key{}
	}
	if ekey.IsZero() {
