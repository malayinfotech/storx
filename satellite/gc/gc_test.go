...
func TestGarbageCollection(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		var deletedPieceID storx.PieceID
...
func TestGarbageCollectionWithCopies(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		encryptedSize, err := encryption.CalcEncryptedSize(int64(len(expectedRemoteData)), storx.EncryptionParameters{
...
func TestGarbageCollection_PendingObject(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		lastPieceCounts := map[storx.NodeID]int64{}
...
func startMultipartUpload(ctx context.Context, t *testing.T, uplink *testplanet.Uplink, satellite *testplanet.Satellite, bucketName string, path storx.Path, data []byte) string {
...
func completeMultipartUpload(ctx context.Context, t *testing.T, uplink *testplanet.Uplink, satellite *testplanet.Satellite, bucketName string, path storx.Path, streamID string) {
	_, found := testuplink.GetMaxSegmentSize(ctx)
	if !found {
		ctx = testuplink.WithMaxSegmentSize(ctx, satellite.Config.Metainfo.MaxSegmentSize)
	}
