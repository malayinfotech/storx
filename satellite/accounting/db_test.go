...
func TestStorageNodeUsage_TwoRollupsInADay(t *testing.T) {
	satellitedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db satellite.DB) {
		...
		rollups[t1] = make(map[storx.NodeID]*accounting.Rollup)
		rollups[t2] = make(map[storx.NodeID]*accounting.Rollup)
...
func makeRollupsAndStorageNodeStorageTallies(nodes []storx.NodeID, start time.Time, days int) (accounting.RollupStats, map[time.Time]map[storx.NodeID]float64, time.Time) {
	rollups := make(accounting.RollupStats)
	tallies := make(map[time.Time]map[storx.NodeID]float64)
	...
	for i := 0; i < days; i++ {
		...
		if rollups[startDay] == nil {
			rollups[startDay] = make(map[storx.NodeID]*accounting.Rollup)
		}
		...
		for _, node := range nodes {
			...
			for h := 0; h < hours; h++ {
				...
				if tallies[intervalEndTime] == nil {
					tallies[intervalEndTime] = make(map[storx.NodeID]float64)
				}
