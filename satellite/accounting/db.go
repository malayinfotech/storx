...
type RollupStats map[time.Time]map[storx.NodeID]*Rollup
...
type StoragenodeAccounting interface {
	...
	SaveTallies(ctx context.Context, latestTally time.Time, nodeData map[storx.NodeID]float64) error
	...
	QueryStorageNodeUsage(ctx context.Context, nodeID storx.NodeID, start time.Time, end time.Time) ([]StorageNodeUsage, error)
