...
func NewService(log *zap.Logger, stripeClient StripeClient, config Config, db DB, walletsDB storxscan.WalletsDB, billingDB billing.TransactionsDB, projectsDB console.Projects, usersDB console.Users, usageDB accounting.ProjectAccounting, usagePrices payments.ProjectUsagePriceModel, usagePriceOverrides map[string]payments.ProjectUsagePriceModel, packagePlans map[string]payments.PackagePlan, bonusRate int64) (*Service, error) {
	var partners []string
...
// InvoiceApplyTokenBalance iterates through customer storxscan wallets and creates invoice credit notes
...
func (service *Service) addCreditNoteToInvoice(ctx context.Context, invoiceID, cusID, wallet string, amount, txID int64) (_ string, err error) {
	...
	lineParam := stripe.CreditNoteLineParams{
		Description: stripe.String("Storxscan Token payment"),
	...
	params := &stripe.CreditNoteParams{
		...
		Lines:   lineParams,
		Memo:    stripe.String("Storxscan Token Payment - Wallet: 0x" + wallet),
	}
...
func (service *Service) createInvoice(ctx context.Context, cusID string, period time.Time) (stripeInvoice *stripe.Invoice, err error) {
	...
	description := fmt.Sprintf("Storx DCS Cloud Storage for %s %d", period.Month(), period.Year())
