...
// ErrService is storxscan service error class.
var ErrService = errs.Class("storxscan service")

type storxscanMetadata struct {
	ReferenceID string
	Wallet      string
	BlockNumber int64
	LogIndex    int
...
// ensures that storxscan implements payments.DepositWallets.
var _ payments.DepositWallets = (*Service)(nil)

// ensures that storxscan implements billing.PaymentType.
var _ billing.PaymentType = (*Service)(nil)

// Service exposes API to interact with storxscan payments provider.
type Service struct {
	log        *zap.Logger
	walletsDB  WalletsDB
...
// NewService creates new storxscan service instance.
...
func (service *Service) Claim(ctx context.Context, userID uuid.UUID) (_ blockchain.Address, err error) {
	...
	service.log.Info("STORX token wallet claimed",
...
// Source defines the billing transaction source for storxscan payments.
func (service *Service) Source() string {
	return "storxscan"
}

// Type defines the billing transaction type for storxscan payments.
func (service *Service) Type() billing.TransactionType {
	return billing.TransactionTypeCredit
}

// GetNewTransactions returns the storxscan payments since the given block number and index as billing transactions type.
func (service *Service) GetNewTransactions(ctx context.Context, _ time.Time, lastPaymentMetadata []byte) ([]billing.Transaction, error) {
	...
	var latestMetadata storxscanMetadata
	if lastPaymentMetadata == nil {
		latestMetadata = storxscanMetadata{}
	...
	for _, payment := range newCachedPayments {
		...
		metadata, err := json.Marshal(storxscanMetadata{
		...
		billingTXs = append(billingTXs, billing.Transaction{
			...
			Description: "Storx token deposit",
