...
func TestPaymentsDBList(t *testing.T) {
	satellitedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db satellite.DB) {
		...
		expected := []storxscan.CachedPayment{
		...
		t.Run("ListWallet", func(t *testing.T) {
			var expectedW []storxscan.CachedPayment
...
func TestPaymentsDBLastBlockNoPayments(t *testing.T) {
	satellitedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db satellite.DB) {
		_, err := db.StorxscanPayments().LastBlock(ctx, payments.PaymentStatusConfirmed)
		require.True(t, errs.Is(err, storxscan.ErrNoPayments))
	})
}
...
func TestPaymentsDBDeletePending(t *testing.T) {
	satellitedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db satellite.DB) {
		...
		payments := []storxscan.CachedPayment{
			blocks[0].NewPayment(paymentLog{
				From:        blockchaintest.NewAddress(),
				To:          blockchaintest.NewAddress(),
				TokenValue:  new(big.Int).SetInt64(testrand.Int63n(1000)),
...
func (block blockHeader) NewPayment(log paymentLog, usdValue currency.Amount, status payments.PaymentStatus) storxscan.CachedPayment {
	return storxscan.CachedPayment{
		From:        log.From,
		To:          log.To,
