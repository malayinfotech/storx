...
func TestNodeAliasCache(t *testing.T) {
	...
	t.Run("auto add nodes", func(t *testing.T) {
		...
		aliases, err := cache.EnsureAliases(ctx, []storx.NodeID{n1, n2})
		require.NoError(t, err)
		require.Equal(t, []metabase.NodeAlias{1, 2}, aliases)

		nx1 := testrand.NodeID()
		aliases, err = cache.EnsureAliases(ctx, []storx.NodeID{nx1, n1, n2})
		require.NoError(t, err)
		require.Equal(t, []metabase.NodeAlias{3, 1, 2}, aliases)

		nodes, err := cache.Nodes(ctx, aliases)
		require.NoError(t, err)
		require.Equal(t, []storx.NodeID{nx1, n1, n2}, nodes)
		...
		for repeat := 0; repeat < 3; repeat++ {
			...
			for k := 0; k < N; k++ {
				group.Go(func() error {
					...
					_, err := cache.EnsureAliases(ctx, []storx.NodeID{n1, n2})
					return err
				})
			}
...
func TestNodeAliasCache_DB(t *testing.T) {
	metabasetest.Run(t, func(ctx *testcontext.Context, t *testing.T, db *metabase.DB) {
		...
		t.Run("auto add nodes", func(t *testing.T) {
			...
			nodes, err := cache.Nodes(ctx, aliases)
			require.NoError(t, err)
			require.Equal(t, []storx.NodeID{n1, n2}, nodes)
		})
	})
}
...
func TestNodeAliasMap(t *testing.T) {
	...
	{
		emptyMap := metabase.NewNodeAliasMap(nil)
		aliases, missing := emptyMap.Aliases([]storx.NodeID{n1, n2, n3})
		require.Empty(t, aliases)
		require.Equal(t, []storx.NodeID{n1, n2, n3}, missing)
	}

	{
		...
		aliases, missing := aggregate.Aliases([]storx.NodeID{n1, n2, n3})
		...
		require.Equal(t, []storx.NodeID{n1, n2, n3}, nodes2)
	...
	testAliases := []struct {
		...
		{
			in:  []storx.NodeID{n1, n3, n2},
			out: []metabase.NodeAlias{1, 3, 2},
		},
		{
			in:      []storx.NodeID{nx2, nx1},
			missing: []storx.NodeID{nx2, nx1},
		},
		{
			in:      []storx.NodeID{n1, nx2, n3, nx1, n2},
			out:     []metabase.NodeAlias{1, 3, 2},
			missing: []storx.NodeID{nx2, nx1},
		},
	}
...
func (db *NodeAliasDB) Ensure(id storx.NodeID) {
	db.mu.Lock()
	defer db.mu.Unlock()
