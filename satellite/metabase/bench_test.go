...
func (s *scenario) run(ctx *testcontext.Context, b *testing.B, db *metabase.DB) {
	if s.redundancy.IsZero() {
		s.redundancy = storx.RedundancyScheme{
					...
					totalUpload.Record(func() {
						...
						for part := 0; part < s.parts; part++ {
							for segment := 0; segment < s.segments-1; segment++ {
								...
								encryptedKey := testrand.BytesInt(storx.KeySize)
								encryptedKeyNonce := testrand.BytesInt(storx.NonceSize)
...
func randPieces(count int, nodes []storx.NodeID) metabase.Pieces {
