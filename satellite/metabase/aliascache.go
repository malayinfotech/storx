...
func (cache *NodeAliasCache) Nodes(ctx context.Context, aliases []NodeAlias) ([]storx.NodeID, error) {
...
func (cache *NodeAliasCache) EnsureAliases(ctx context.Context, nodes []storx.NodeID) ([]NodeAlias, error) {
	latest := cache.getLatest()
...
func (cache *NodeAliasCache) Aliases(ctx context.Context, nodes []storx.NodeID) ([]NodeAlias, error) {
	latest := cache.getLatest()
...
func (cache *NodeAliasCache) ensure(ctx context.Context, missing ...storx.NodeID) (_ *NodeAliasMap, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cache *NodeAliasCache) refresh(ctx context.Context, missingNodes []storx.NodeID, missingAliases []NodeAlias) (_ *NodeAliasMap, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (cache *NodeAliasCache) EnsurePiecesToAliases(ctx context.Context, pieces Pieces) (_ AliasPieces, err error) {
	...
	nodes := make([]storx.NodeID, len(pieces))
...
func (m *NodeAliasMap) setNode(alias NodeAlias, value storx.NodeID) {
	if int(alias) >= len(m.node) {
		m.node = append(m.node, make([]storx.NodeID, int(alias)-len(m.node)+1)...)
	}
	m.node[alias] = value
}
...
func (m *NodeAliasMap) Node(alias NodeAlias) (x storx.NodeID, ok bool) {
	if int(alias) >= len(m.node) {
		return storx.NodeID{}, false
	}
	v := m.node[alias]
	return v, !v.IsZero()
}
...
func (m *NodeAliasMap) Alias(node storx.NodeID) (x NodeAlias, ok bool) {
	x, ok = m.alias[node]
	return x, ok
}

// Nodes returns NodeID-s for the given aliases and aliases that are not in this map.
func (m *NodeAliasMap) Nodes(aliases []NodeAlias) (xs []storx.NodeID, missing []NodeAlias) {
	xs = make([]storx.NodeID, 0, len(aliases))
...
func (m *NodeAliasMap) Aliases(nodes []storx.NodeID) (xs []NodeAlias, missing []storx.NodeID) {
...
func (m *NodeAliasMap) ContainsAll(nodeIDs []storx.NodeID, nodeAliases []NodeAlias) bool {
