...
func Browser(t *testing.T, ctx *testcontext.Context, planet *testplanet.Planet, fn func(*rod.Browser)) {
	showBrowser := os.Getenv("STORX_TEST_SHOW_BROWSER") != ""
	slowBrowser := os.Getenv("STORX_TEST_SHOW_BROWSER") == "slow"
	...
	if browserHost := os.Getenv("STORX_TEST_BROWER_HOSTPORT"); browserHost != "" {
	...
	if browserBin := os.Getenv("STORX_TEST_BROWSER"); browserBin != "" {
