...
func TestOnboarding_WizardCLIFlow(t *testing.T) {
	uitest.Run(t, func(t *testing.T, ctx *testcontext.Context, planet *testplanet.Planet, browser *rod.Browser) {
		...
		require.Equal(t, *windowsBinaryLink, "https://github.com/storx/storx/releases/latest/download/uplink_windows_amd64.zip")
		...
		require.NoError(t, err)
		require.Equal(t, *linuxAMDBinaryLink, "https://github.com/storx/storx/releases/latest/download/uplink_linux_amd64.zip")
		linuxARMBinaryLink, err := page.MustElementX("(//a[text()=\" Linux ARM Uplink Binary \"])").Attribute("href")
		require.NoError(t, err)
		require.Equal(t, *linuxARMBinaryLink, "https://github.com/storx/storx/releases/latest/download/uplink_linux_arm.zip")
		...
		require.NoError(t, err)
		require.Equal(t, *macOSBinaryLink, "https://github.com/storx/storx/releases/latest/download/uplink_darwin_amd64.zip")
...
func TestOnboarding_WelcomeScreenEncryption(t *testing.T) {
	uitest.Run(t, func(t *testing.T, ctx *testcontext.Context, planet *testplanet.Planet, browser *rod.Browser) {
		...
		require.NoError(t, err)
		require.Equal(t, "https://docs.storx/concepts/encryption-key/design-decision-server-side-encryption", *serverSideEncLink)
		endToEndEncLink, err := page.MustElement("[aria-roledescription=end-to-end-encryption-link]").Attribute("href")
		require.NoError(t, err)
		require.Equal(t, "https://docs.storx/concepts/encryption-key/design-decision-end-to-end-encryption", *endToEndEncLink)
	})
}
