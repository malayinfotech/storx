...
func TestClientWalletsClaim(t *testing.T) {
	storxscantest.Run(t, func(t *testing.T, ctx *testcontext.Context, planet *testplanet.Planet, stack *storxscantest.Stack) {
		...
		addr, err := planet.Satellites[0].API.Payments.StorxscanClient.ClaimNewEthAddress(ctx)
		require.NoError(t, err)
...
func TestClientPayments(t *testing.T) {
	storxscantest.Run(t, func(t *testing.T, ctx *testcontext.Context, planet *testplanet.Planet, stack *storxscantest.Stack) {
		...
		// claim wallet
		_, err = planet.Satellites[0].API.Payments.StorxscanClient.ClaimNewEthAddress(ctx)
		require.NoError(t, err)
		...
		pmnts, err := planet.Satellites[0].API.Payments.StorxscanClient.Payments(ctx, 0)
		require.NoError(t, err)
		require.Equal(t, block.Number().Int64(), pmnts.LatestBlock.Number)
		require.Len(t, pmnts.Payments, 1)

		expected := storxscan.Payment{
